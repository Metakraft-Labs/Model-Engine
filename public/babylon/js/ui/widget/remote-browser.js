...
export class RemoteBrowser extends ImageArea {
    constructor(scene) {
        super(scene);
        this.size = 0.3;
        this.position = new BABYLON.Vector3(0, 0.15, -0.05);
        this.depth = 0;
        this.maxDepth = 0;
        this.inputForm = new InputForm("Enter");
    }
    endpoint() {
        return VRSPACEUI.contentBase + "/api/webbrowser";
    }
    async available() {
        ...
        if (depth && maxDepth) {
            console.log(
                "Depth: " +
                    ...
                    maxDepth +
                    " clicked " +
                    headers.get("clicked-element") +
                    " active " +
                    headers.get("active-element") +
                    " windows " +
                    windows,
            );
            this.depth = depth;
    ...
    show() {
        ...
        this.inputForm.size = 0.1;
        let formPlane = this.inputForm.init();
        formPlane.parent = this.group;
        formPlane.position = new BABYLON.Vector3(0, -0.12, -0.05);
