...
export class WorldEditor {
    ...
    constructor(world, fileInput) {
        ...
        this.worldManager.loadErrorHandler = (object, exception) =>
            this.loadingFailed(object, exception);
    ...
    endpoint() {
        return VRSPACEUI.contentBase + "/api/sketchfab";
    }
    /**
  ...
  */
    createButtons() {
        this.moveButton = this.makeAButton(
            "Move",
            this.contentBase + "/content/icons/move.png",
            o => this.take(o.VRObject, o.position),
        );
        ...
        this.gizmoButton = this.makeAButton(
            "Rotate/Scale",
            this.contentBase + "/content/icons/rotate-resize.png",
            o => this.createGizmo(o),
        );
        this.alignButton = this.makeAButton(
            "Align",
            this.contentBase + "/content/icons/download.png",
            o => this.alignObject(o),
        );
        this.alignButton = this.makeAButton(
            "Upright",
            this.contentBase + "/content/icons/upload.png",
            o => this.upright(o),
        );
        this.copyButton = this.makeAButton(
            "Copy",
            this.contentBase + "/content/icons/copy.png",
            o => this.copyObject(o),
        );
        this.deleteButton = this.makeAButton(
            "Remove",
            this.contentBase + "/content/icons/delete.png",
            o => this.removeObject(o),
        );
        this.searchButton = this.makeAButton(
            "Search",
            this.contentBase + "/content/icons/zoom.png",
        );
        this.generateButton = this.makeAButton(
            "Generate",
            this.contentBase + "/content/icons/magic-wand.png",
        );
        ...
        this.loadButton.onPointerDownObservable.add(() => {
            this.load();
        });
        this.generateButton.onPointerDownObservable.add(() => {
            this.generate();
        });
    ...
    objectLoaded(vrObject, rootMesh) {
        console.log(
            "WorldEditor loaded: " + vrObject.className + " " + vrObject.id + " " + vrObject.mesh,
        );
        if (vrObject.properties && vrObject.properties.editing == this.worldManager.VRSPACE.me.id) {
            ...
            if (!vrObject.scale) {
                ...
                setTimeout(() => {
                    ...
                    this.worldManager.VRSPACE.sendEvent(vrObject, {
    ...
    createGizmo(obj) {
        ...
        this.gizmo.onScaleBoxDragEndObservable.add(() => {
            this.worldManager.VRSPACE.sendEvent(obj.VRObject, {
        ...
        var resizeHandler = this.scene.onPointerObservable.add(pointerInfo => {
            ...
            if (pointerInfo.type == BABYLON.PointerEventTypes.POINTERUP) {
                ...
                if (
                    ...
                    VRSPACEUI.findRootNode(pointerInfo.pickInfo.pickedMesh) == obj
                ) {
                    ...
                    scale = Math.max(scale, obj.scaling.y * 0.2);
        ...
        var rotateHandler = this.scene.onPointerObservable.add(pointerInfo => {
            ...
            if (pointerInfo.type == BABYLON.PointerEventTypes.POINTERUP) {
                ...
                if (
                    ...
                    var result = BABYLON.Quaternion.FromEulerVector(obj.rotation)
                        .multiply(quat)
                        .toEulerAngles();
                    ...
                    var dxz = new BABYLON.Vector3(dest.x, 0, dest.z)
                        .subtract(new BABYLON.Vector3(point.x, 0, point.z))
                        .length();
                    if (Math.abs(dxz) > Math.abs(dy * 3)) {
                        // mostly horizontal movement, rotation only around y
                        console.log("Y rotation");
                        this.worldManager.VRSPACE.sendEvent(obj.VRObject, {
                            rotation: { x: obj.rotation.x, y: result.y, z: obj.rotation.z },
                        });
                    } else {
                        // rotating around all axes
                        this.worldManager.VRSPACE.sendEvent(obj.VRObject, {
                            rotation: { x: result.x, y: result.y, z: result.z },
                        });
                    }
                }
            }
    ...
    upright(obj) {
        this.clearGizmo();
        this.worldManager.VRSPACE.sendEvent(obj.VRObject, {
            rotation: { x: 0, y: obj.rotation.y, z: 0 },
        });
    }
    ...
    copyObject(obj) {
        ...
        this.activeButton = null;
        this.displayButtons(true);
        this.createSharedObject(vrObject.mesh, {
                    ...
                    case BABYLON.PointerEventTypes.POINTERDOWN:
                        if (this.activeButton) {
                            ...
                            if (pickedRoot.VRObject && this.activeButton.isVisible) {
                                ...
                                console.log(
                                    "Manipulating shared object " +
                                        pickedRoot.VRObject.id +
                                        " " +
                                        pickedRoot.name,
                                );
    ...
    take(vrObject, position) {
        ...
        try {
            ...
            if (!position) {
                ...
                var forwardLower = forwardDirection.add(new BABYLON.Vector3(0, -0.5, 0));
      ...
      */
            ...
            var forwardDirection = VRSPACEUI.hud.camera.getForwardRay(
                targetDirection.length(),
            ).direction;

            var rotationMatrix = new BABYLON.Matrix();
            BABYLON.Matrix.RotationAlignToRef(
                forwardDirection.normalizeToNew(),
                targetDirection.normalizeToNew(),
                rotationMatrix,
            );
            var quat = BABYLON.Quaternion.FromRotationMatrix(rotationMatrix);
            ...
            if (vrObject.rotation) {
                ...
                quat = BABYLON.Quaternion.Inverse(VRSPACEUI.hud.camera.absoluteRotation).multiply(
                    quat,
                );
                //} else {
    ...
    sendPos(obj) {
        ...
        this.worldManager.VRSPACE.sendEvent(obj, {
    ...
    */
    ...
    endManipulation() {
        try {
            if (this.carrying && this.startData) {
                ...
                let distance = this.world.xrHelper
                    .leftArmPos()
                    .subtract(this.world.xrHelper.rightArmPos())
                    .length();
                ...
                let curQuat = BABYLON.Quaternion.FromEulerAngles(
                    this.carrying.rotation.x,
                    this.carrying.rotation.y,
                    this.carrying.rotation.z,
                );
    ...
    handleSqueeze(value, side) {
        try {
            this.clearGizmo();
            let bothOn =
                this.world.xrHelper.squeeze.left.value == 1 &&
                this.world.xrHelper.squeeze.right.value == 1;
            let bothOff =
                this.world.xrHelper.squeeze.left.value == 0 &&
                this.world.xrHelper.squeeze.right.value == 0;
            if (value == 1) {
