...
export class ModelGenerator {
    ...
    show() {
        ...
        this.refineButton = this.makeAButton(
            "Refine",
            this.contentBase + "/content/icons/magic-wand.png",
            () => this.refine(),
        );
        this.styleButton = this.makeAButton(
            "Style",
            this.contentBase + "/content/icons/magic-wand.png",
            () => this.style(),
        );
        this.rigButton = this.makeAButton(
            "Rig",
            this.contentBase + "/content/icons/magic-wand.png",
            () => this.rig(),
        );
        this.animateButton = this.makeAButton(
            "Animate",
            this.contentBase + "/content/icons/magic-wand.png",
            () => this.animate(),
        );
    ...
    style() {
        if (this.legoButton) {
            ...
            VRSPACEUI.hud.newRow(); // stops speech recognition
            this.legoButton = this.makeAButton(
                "Lego",
                this.contentBase + "/content/icons/magic-wand.png",
                () => this.doStyle("lego"),
                true,
            );
            this.voxelButton = this.makeAButton(
                "Voxel",
                this.contentBase + "/content/icons/magic-wand.png",
                () => this.doStyle("voxel"),
                true,
            );
            this.voronoiButton = this.makeAButton(
                "Voronoi",
                this.contentBase + "/content/icons/magic-wand.png",
                () => this.doStyle("voronoi"),
                true,
            );
        }
    }
    ...
    doStyle(name) {
        ...
        fetch("/api/metakraft/style?style=" + name + "&id=" + this.vrObject.id, {
            method: "POST",
        })
            .then(res => res.text().then(json => console.log(json)))
            .finally(() => this.world.loadingStop("styling"));
    }
    /**
    ...
    doPrompt(text) {
        ...
        fetch(
            ...
            .then(res => res.text().then(json => console.log(json)))
            .finally(() => this.world.loadingStop("generated object"));
    }

    makeAButton(text, imageUrl, action, enabled = false) {
        var button = VRSPACEUI.hud.addButton(
            text,
            imageUrl,
            () => {
    ...
    isMine(vrObject) {
        return (
            vrObject.properties &&
            this.world.worldManager.VRSPACE.me.id == vrObject.properties.clientId
        );
    }
}
