...
export class VRSpace {
  ...
  */
  ...
  @param url optional websocket url, defaults to /client on the same server
   */
    connect(url) {
        if (!url) {
            ...
            url = webSocketProtocol + ":" + url.substring(start, end) + "/client"; // ws://localhost:8080
        }
   ...
   */
    async _createSharedObject(command, obj, className) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.call(
                '{"command":{"' + command + '":{"objects":[{"' + className + '":' + json + "}]}}}",
                response => {
   ...
   */
    deleteSharedObject(obj, callback) {
        this.call(
            '{"command":{"Remove":{"objects":[{"' + obj.className + '":' + obj.id + "}]}}}",
            response => {
                if (callback) {
                    callback(obj);
                }
        ...
        if (this.me != null) {
            this.send(
                '{"object":{"' +
                    ...
                    this.me.id +
                    '},"changes":{' +
                    this.stringifyPair(field, value) +
                    "}}",
            );
        } else {
