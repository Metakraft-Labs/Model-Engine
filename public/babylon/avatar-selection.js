...
export class AvatarSelection extends World {
    ...
    or "/content/skybox/eso_milkyway/eso0932a.jpg" (panoramic)*/
        ...
        this.api = new VRSpaceAPI(VRSPACEUI.contentBase + "/api");
        this.tokens = {};
    }
    async createSkyBox() {
    ...
    async createUI() {
        ...
        if (this.enableLogin) {
            this.loginForm.position = new BABYLON.Vector3(0.25, 0.6, -1);
    ...
    isSelectableMesh(mesh) {
        return (
            ...
            (mesh.name &&
                (mesh.name.startsWith("Button") || mesh.name.startsWith("PortalEntrance"))) ||
            ...
            if (this.xrHelper.controller.right) {
                if (this.mirror) {
                    var rightPos = this.calcControllerPos(
                        this.character.body.rightArm,
                        this.xrHelper.controller.right,
                    );
    ...
    createSelection(selectionCallback) {
        ...
        VRSPACEUI.listMatchingFilesAsync(this.characterDir()).then(folders => {
            ...
            buttons.group.position = new BABYLON.Vector3(0.3, 2.2, this.buttonsZ);
            buttons.select(0);
    ...
    animationButtons(avatar) {
        ...
        this.animationSelection = new Buttons(this.scene, "Animations", names, name =>
    ...
    addCharacterButtons() {
        ...
        mirrorButton.onPointerDownObservable.add(() => {
            if (mirrorButton.text == "Mirroring") {
                ...
                this.mirror = false;
                this.character.parentMesh.rotationQuaternion = BABYLON.Quaternion.RotationAxis(
                    BABYLON.Axis.Y,
                    Math.PI,
                );
            } else {
    ...
    showActiveUsers() {
        fetch(this.api.endpoint.worlds + "/users").then(response =>
    ...
    async enterWorld(worldUrl, worldName, avatarUrl = this.avatarUrl(), worldScript = "world.js") {
        ...
        import(worldUrl + "/" + worldScript).then(world => {
            ...
            world.WORLD.init(this.engine, worldName, this.scene, afterLoad, worldUrl + "/").then(
                newScene => {
                    try {
