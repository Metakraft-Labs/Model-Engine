...
export function mat2SetColumn3(m: Mat2, columnIndex, column: Vec2, result = new Mat2()): Mat2 {
...
export function mat2SetRow3(m: Mat2, rowIndex, row: Vec2, result = new Mat2()): Mat2 {
    const re = result.set(m.elements).elements;
...
export function mat2Equals(a: Mat2, b: Mat2, tolerance = EPSILON) {
    for (let i = 0; i < NUM_ELEMENTS; i++) {
...
export function mat2MultiplyByScalar(a: Mat2, b, result: Mat2 = new Mat2()): Mat2 {
    for (let i = 0; i < NUM_ELEMENTS; i++) {
...
export function mat2Mix(a: Mat2, b: Mat2, t, result = new Mat2()): Mat2 {
    const s = 1 - t;
    for (let i = 0; i < NUM_ELEMENTS; i++) {
...
export function mat2ToArray(a: Mat2, array: Float32Array | number[], offset = 0) {
    for (let i = 0; i < NUM_ELEMENTS; i++) {
        array[offset + i] = a.elements[i];
    }
}

export function mat2ToString(a: Mat2) {
    return toSafeString(a.elements);
}
export function mat2Parse(text, result = new Mat2()): Mat2 {
