...
export function vec2Equals(a: Vec2, b: Vec2, tolerance = EPSILON) {
...
export function vec2Mix(a: Vec2, b: Vec2, t, result = new Vec2()): Vec2 {
    const s = 1 - t;
...
export function vec2ToArray(a: Vec2, array: Float32Array | number[], offset = 0) {
    array[offset + 0] = a.x;
    array[offset + 1] = a.y;
}

export function vec2ToString(a: Vec2) {
    return toSafeString([a.x, a.y]);
}
export function vec2Parse(text, result = new Vec2()): Vec2 {
