...
export function vec3Equals(a: Vec3, b: Vec3, tolerance = EPSILON) {
    return (
...
export function vec3MultiplyByScalar(a: Vec3, b, result = new Vec3()): Vec3 {
...
export function vec3Mix(a: Vec3, b: Vec3, t, result = new Vec3()): Vec3 {
    const s = 1 - t;
...
export function vec3ToArray(a: Vec3, array: Float32Array | number[], offset = 0) {
    array[offset + 0] = a.x;
    array[offset + 1] = a.y;
    array[offset + 2] = a.z;
}
export function vec3ToString(a: Vec3) {
    return toSafeString([a.x, a.y, a.z]);
}
export function vec3Parse(text, result = new Vec3()): Vec3 {
...
export function hexToRGB(hex, result = new Vec3()): Vec3 {
    hex = Math.floor(hex);
