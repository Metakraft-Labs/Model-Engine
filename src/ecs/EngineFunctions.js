import { getMutableState, getState, HyperFlux } from "../hyperflux";
import { ECSState } from "./ECSState";
import { executeSystem, SystemDefinitions } from "./SystemFunctions";
import {
 ...
 */
export const executeSystems = elapsedTime => {
    ...
    ecsState.deltaSeconds.set(
        Math.max(
            0.001,
            Math.min(TimerConfig.MAX_DELTA_SECONDS, elapsedSeconds - ecsState.elapsedSeconds.value),
        ),
    );
 ...
 */
export const executeFixedSystem = systemUUID => {
    const start = nowMilliseconds();
    let timeUsed = 0;
    ...
    if (simulationDelay < simulationTimestep) {
        ecsState.simulationTime.set(
            Math.floor(frameTime / simulationTimestep) * simulationTimestep,
        );
    ...
    while (simulationDelay > simulationTimestep && !timeout) {
        ...
        if (simulationDelay >= maxSimulationDelay) {
            ...
            ecsState.simulationTime.set(
                t => Math.floor(frameTime / simulationTimestep) * simulationTimestep,
            );
            break;
        }
    }
