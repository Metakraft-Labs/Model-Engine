...
export class WebLayer3D extends Object3D {
    ...
    constructor(element) {
        ...
        const geometry = this._webLayer.isMediaElement
            ? WebLayer3D.GEOMETRY
            : WebLayer3D.FLIPPED_GEOMETRY;
        ...
        if (_layer.isMediaElement) {
            ...
            if (!t || (t.image && media.src !== t.image.src)) {
                ...
                t = _layer.isVideoElement
                    ? new VideoTexture(media)
                    : _layer.isCanvasElement
                      ? new CanvasTexture(media)
    ...
    async refresh(recurse = false) {
        ...
        for (const c of this._webLayer.childLayers) {
            const child = this.container.manager.layersByElement.get(
                WebRenderer.getClosestLayer(c.element)?.element,
            );
            if (!child) continue;
    ...
    updateLayout() {
        ...
        if (this._camera) {
            ...
            this._viewZ = Math.abs(
                this.contentMesh
                    .getWorldPosition(scratchVector)
                    .applyMatrix4(this._camera.matrixWorldInverse).z,
            );
    ...
    updateContent() {
        ...
        if (texture && material.map !== texture) {
            ...
            const aspect = Math.abs(
                ((contentScale.x * this.scale.x) / contentScale.y) * this.scale.y,
            );
    ...
    querySelectorAll(selector) {
        ...
        return Array.from(elements)
            .map(e => this.container.manager.layersByElement.get(e))
            .filter(l => l);
    }
    ...
    _refreshMediaBounds() {
        if (this._webLayer.isMediaElement) {
            ...
            const media = this.element & HTMLImageElement;
            const texture = this.texture;
            ...
            switch (objectFit) {
                case "none":
                    texture.repeat
                        .set(viewWidth / naturalWidth, viewHeight / naturalHeight)
                        .clampScalar(0, 1);
                    break;
                case "contain":
                case "scale-down":
    ...
    _updateDOMLayout() {
        ...
        if (cssTransform) {
            ...
            this.domLayout.matrix.decompose(
                this.domLayout.position,
                this.domLayout.quaternion,
                this.domLayout.scale,
            );
        }
    }
}
