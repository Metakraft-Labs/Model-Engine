...
if (!IN_WORKER) {
    ...
    class PendingTextureRequest {
        ...
        uploadImageData(webglFormat, buffer, mipLevels) {
            ...
            gl.texParameteri(
                ...
                mipLevels.length > 1 || webglFormat.uncompressed
    ...
    function transcode(id, arrayBuffer, supportedFormats, allowSeparateAlpha) {
        ...
        for (let mipLevel = 0; mipLevel < levels; ++mipLevel) {
            let transcodeSize = basisFile.getImageTranscodedSizeInBytes(
                IMAGE_INDEX,
                mipLevel,
                basisFormat,
            );
        ...
        for (let mipLevel of mipLevels) {
            ...
            if (
                !basisFile.transcodeImage(levelData, IMAGE_INDEX, mipLevel.level, basisFormat, 1, 0)
            ) {
                basisFileFail(id, basisFile, "transcodeImage failed");
                return;
            }
            if (needsSecondaryAlpha) {
                let alphaLevelData = new Uint8Array(
                    alphaTranscodeData.buffer,
                    mipLevel.offset,
                    mipLevel.size,
                );
                if (
