...
export class WebLayer {
    ...
    constructor(managerBase, element, eventCallback) {
        ...
        this.isMediaElement =
            this.isVideoElement || element.nodeName === "IMG" || element.nodeName === "CANVAS";
    ...
    // prerasterizedRange = []
    // prerasterizedImages: Map = new Map()
    ...
    get previousDOMState() {
        return this.previousDOMStateKey
            ? this.manager.getLayerState(this.previousDOMStateKey)
            : undefined;
    }

    get desiredDOMState() {
        return this.desiredDOMStateKey
            ? this.manager.getLayerState(this.desiredDOMStateKey)
            : undefined;
    }

    get currentDOMState() {
        return this.currentDOMStateKey
            ? this.manager.getLayerState(this.currentDOMStateKey)
            : undefined;
    }
    ...
    update() {
        ...
        const prev =
            this.previousDOMState?.texture?.ktx2Url ?? this.previousDOMState?.texture?.canvas;
        const current =
            this.currentDOMState?.texture?.ktx2Url ?? this.previousDOMState?.texture?.canvas;
