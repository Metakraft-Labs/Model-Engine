...
export class WebLayerManagerBase {
    ...
    async exportCache(states = this.getActiveStateHashes()) {
        ...
        let textureData = await this.store.textures.bulkGet(
            stateData.map(v => v.textureHash).filter(v => typeof v === "string"),
        );
    ...
    async loadIntoStore(data) {
        ...
        for (const t of data.textureData) {
            ...
            if (!texture.ktx2Url && t.texture)
                texture.ktx2Url = URL.createObjectURL(
                    new Blob([t.texture], { type: "image/ktx2" }),
                );
        }
        ...
        if (
            ...
            if (storedTexture?.texture && !textureData.canvas) {
                ...
                if (!textureData.canvas) {
                    textureData.ktx2Url = URL.createObjectURL(
                        new Blob([data.buffer], { type: "image/ktx2" }),
                    );
                }
            }
        }
    ...
    async compressTexture(textureHash) {
        ...
        try {
            ...
            console.error(
                `KTX2 encoding failed for image (${canvas.width}, ${canvas.height}) `,
                error,
            );
    ...
    _runTasks = () => {
        ...
        while (
            serializeQueue.length > 0 &&
            this.serializePendingCount < this.MAX_SERIALIZE_TASK_COUNT
        ) {
        ...
        while (
            rasterizeQueue.length > 0 &&
            this.rasterizePendingCount < this.MAX_RASTERIZE_TASK_COUNT
        ) {
    ...
    async serialize(layer) {
        ...
        if (transform && transform !== "none") {
            ...
            cssTransform = parseCSSTransform(
                computedStyle,
                width,
                height,
                pixelSize,
        ...
        if (layer.isMediaElement) {
            ...
            const parentsHTML = getParentsHTML(
                layer,
                fullWidth,
                fullHeight,
                textureWidth,
            ...
            // @ts-ignore
            const stateHashBuffer = await crypto.subtle.digest(
                "SHA-1",
                WebRenderer.textEncoder.encode(hashComponents),
            );
            const stateHash =
        ...
        result.needsRasterize =
            !layer.isMediaElement && fullWidth * fullHeight > 0 && !data.texture?.hash;
        result.svgUrl =
            result.needsRasterize && svgDoc
    ...
    async rasterize(stateHash, svgUrl) {
        ...
        const hashCanvas = await this.rasterizeToCanvas(
            svgImage,
            sourceWidth,
            sourceHeight,
            30,
        ...
        const textureHash =
            bufferToHex(textureHashBuffer) + "?w=" + textureWidth + ";h=" + textureHeight;
        ...
        setTimeout(
            ...
            ((500 + 0.1 * 1000) * 2) ^ stateData.renderAttempts,
        );
