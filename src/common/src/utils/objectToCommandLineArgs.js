...
export function objectToArgs(obj, prefix = "") {
    const args = [];
...
export function argsToObject(args) {
    ...
    for (let i = 0; i < args.length; i += 2) {
        ...
        const keys = arg.slice(2).split("_");
        const type = keys.pop();
        const parsedKeys = keys.map((key, i) =>
            isNaN(Number.parseFloat(keys[i])) ? key.toLowerCase() : Number(key),
        );
