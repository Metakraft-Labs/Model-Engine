...
import { getMutableState, none } from "../../../hyperflux";
import { NetworkState } from "../NetworkState";

function createPeer(network, peerID, peerIndex, userID, userIndex) {
...
function destroyPeer(network, peerID) {
    if (!network.peers[peerID])
        return console.warn(
            `[NetworkPeerFunctions]: tried to remove client with peerID ${peerID} that doesn't exit`,
        );
...
function getCachedActionsForPeer(toPeerID) {
    ...
    for (const action of Engine.instance.store.actions.cached) {
        ...
        if (action.$to === "all" || action.$to === toPeerID)
            cachedActions.push({ ...action, $stack });
    }
