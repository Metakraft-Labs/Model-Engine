...
import { Engine, getOptionalComponent, removeEntity, setComponent, UUIDComponent } from "../../ecs";
import {
    defineState,
    dispatchAction,
    getMutableState,
...
export const EntityNetworkState = defineState({
    ...
    receptors: {
        ...
        onRequestAuthorityOverObject: WorldNetworkAction.requestAuthorityOverObject.receive(
            action => {
                getMutableState(EntityNetworkState)[action.entityUUID].requestingPeerId.set(
                    action.newAuthority,
                );
            },
        ),

        onTransferAuthorityOfObject: WorldNetworkAction.transferAuthorityOfObject.receive(
            action => {
...
const EntityNetworkReactor = props => {
    ...
    const userConnected =
        !!useHookstate(getMutableState(NetworkWorldUserState)[ownerID]).value || isOwner;
...
const OwnerPeerReactor = props => {
    ...
    useEffect(() => {
        return () => {
            ...
            if (
                ownerPeer !== Engine.instance.store.peerID &&
                Engine.instance.store.userID === state.ownerId.value
            ) {
                const lowestPeer = [...networkState.users[Engine.instance.userID].value].sort(
                    (a, b) => (a > b ? 1 : -1),
                )[0];
                ...
                dispatchAction(
                    WorldNetworkAction.spawnEntity({
                        ...
                        authorityPeerId: networkState.users[Engine.instance.userID].value.includes(
                            ownerPeer,
                        )
                            ? undefined
