 ...
 * @param  {any} component
 */
export const writeComponent = component => {
    ...
    return (v, entity, ignoreHasChanged = false) => {
...
export const writeVector3 =
    vector3 =>
    (v, entity, ignoreHasChanged = false) => {
        const rewind = rewindViewCursor(v);
        const writeChangeMask = spaceUint8(v);
        let changeMask = 0;
        let b = 0;

        changeMask |= writePropIfChanged(v, vector3.x, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
        changeMask |= writePropIfChanged(v, vector3.y, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
        changeMask |= writePropIfChanged(v, vector3.z, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
...
export const writeVector4 =
    ...
    (v, entity, ignoreHasChanged = false) => {
        ...
        changeMask |= writePropIfChanged(v, vector4.x, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
        changeMask |= writePropIfChanged(v, vector4.y, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
        changeMask |= writePropIfChanged(v, vector4.z, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
        changeMask |= writePropIfChanged(v, vector4.w, entity, ignoreHasChanged)
            ? 1 << b++
            : b++ && 0;
...
export const writeCompressedRotation =
    ...
    (v, entity, ignoreHasChanged = false) => {
        ...
        if (changeMask > 0) {
            ...
            for (let i = 0; i < 4; i++) {
                let element = getVector4IndexBasedComponentValue(vector4, entity, i);
...
export const writeEntity = (v, networkId, ownerIndex, entity, serializationSchema) => {
    const rewind = rewindViewCursor(v);
    ...
    return (
        (changeMask > 0 &&
            ...
            writeOwnerIndex(ownerIndex) &&
            writeChangeMask(changeMask)) ||
        rewind()
    );
};

export const writeEntities = (v, network, entities) => {
    ...
    for (let i = 0, l = entities.length; i < l; i++) {
        ...
        const networkId = NetworkObjectComponent.networkId[entity];
...
export const writeMetadata = (v, network, peerID) => {
...
export const createDataWriter = (size = 100000) => {
    ...
    return (network, peerID, entities) => {
