...
const initialize8thwallDevice = async existingCanvas => {
    if (existingCanvas) {
...
const viewerInputSource = {
    ...
    get targetRaySpace() {
        ...
        return new XRSpace(camera.position, camera.quaternion);
    },
    gamepad: {
        axes: [0, 0],
        buttons: {},
...
const onTouchStart = ev => {
    viewerInputSource.gamepad.axes = [
...
const revertXRSessionFunctions = () => {
    ...
    navigator.xr
        ...
        .then(supported =>
            getMutableState(XRState).supportedSessionModes["immersive-ar"].set(supported),
        );
};
...
const execute = () => {
    ...
    getState(XRState).xrFrame = new XRFrameProxy();
};

const reactor = () => {
    const hasPersistentAnchor = useQuery([PersistentAnchorComponent]).length;
    const arSupported = useHookstate(
        getMutableState(XRState).supportedSessionModes["immersive-ar"],
    );
