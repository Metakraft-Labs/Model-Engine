...
import { dispatchAction, getMutableState } from "../../hyperflux";
...
export const setupXRSession = async requestedMode => {
    ...
    const framebufferScaleFactor =
        xrSession.interactionMode === "screen-space" && xrSession.domOverlayState?.type === "screen"
            ? 0.5
            : 1.2;
...
export const getReferenceSpaces = xrSession => {
    ...
    xrSession.requestReferenceSpace("local-floor").then(space => {
        ...
        if ("addEventListener" in space)
            space.addEventListener("reset", onLocalFloorReset, { once: true, passive: true });
 ...
 * @returns
 */
export const requestXRSession = createHookableFunction(async (action = {}) => {
    const xrState = getMutableState(XRState);
 ...
 * @returns
 */
export const xrSessionChanged = createHookableFunction(_action => {});
