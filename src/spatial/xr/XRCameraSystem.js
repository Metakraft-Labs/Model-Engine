 ...
 */
function updateProjectionFromCameraArrayUnion(camera) {
...
function updateCameraFromXRViewerPose() {
    ...
    if (pose) {
        ...
        cameraTransform.position.copy(pose.transform.position);
        cameraTransform.rotation.copy(pose.transform.orientation);
        ...
        for (let i = 0; i < views.length; i++) {
            ...
            viewCamera.position.copy(view.transform.position);
            viewCamera.quaternion.copy(view.transform.orientation);
...
const execute = () => {
    ...
    getMutableState(XRState).viewerPose.set(
        ReferenceSpace.localFloor && xrFrame.getViewerPose(ReferenceSpace.localFloor),
    );
};
