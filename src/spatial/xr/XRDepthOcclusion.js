...
const XRDepthOcclusionMaterials = [];
 ...
 */
function addDepthOBCPlugin(material, depthMap) {
    const mat = material;
    if (!mat) return;
...
function removeDepthOBCPlugin(material) {
    const mat = material;
    if (!mat?.userData?.DepthOcclusionPlugin) return;
...
function updateDepthMaterials(frame, referenceSpace, depthTexture) {
        ...
        for (const view of viewerPose.views) {
            ...
            if (depthInfo) {
                if (!xrState.depthDataTexture.value) {
                    xrState.depthDataTexture.set(
                        new DepthDataTexture(depthInfo.width, depthInfo.height),
                    );
                }
 ...
 */
function updateUniforms(materials, depthInfo) {
    ...
    for (const material of materials) {
        if (material.userData.DepthOcclusionPlugin && material.shader) {
            ...
            material.shader.uniforms.uUvTransform.value
                .fromArray(normTextureFromNormViewMatrix)
                .invert();
...
const _createDepthDebugCanvas = enabled => {
    if (!enabled) return;
...
const execute = () => {
    ...
    const xrFrameD = xrFrame & getDepthInformationType;
    depthSupported = typeof xrFrameD?.getDepthInformation === "function";
    if (!depthSupported) return;
    XRDepthOcclusion.updateDepthMaterials(xrFrame, ReferenceSpace.origin, depthTexture);
};
