...
export const updateHitTest = entity => {
    ...
    transform.position.copy(pose.transform.position);
    transform.rotation.copy(pose.transform.orientation);
};

export const updateAnchor = entity => {
    const xrFrame = getState(XRState).xrFrame;
    const anchor = getComponent(entity, XRAnchorComponent).anchor;
    const transform = getComponent(entity, TransformComponent);
    const pose =
        ReferenceSpace.localFloor && xrFrame.getPose(anchor.anchorSpace, ReferenceSpace.localFloor);
    if (pose) {
        transform.position.copy(pose.transform.position);
        transform.rotation.copy(pose.transform.orientation);
    }
...
const getTargetWorldSize = transform => {
    ...
    const dist = _plane
        ...
        .distanceToPoint(viewerPose.transform.position);
    ...
    return MathUtils.clamp(
        Math.pow(normalizedDist, 2) * scalingFactor,
        minDollhouseScale,
        maxDollhouseScale,
    );
};

export const updateScenePlacement = scenePlacementEntity => {
...
const Reactor = () => {
    useEffect(() => {
        ...
        const mergedGeometry = mergeBufferGeometries([
            scenePlacementRingGeom,
            pinSphereGeometry,
            pinConeGeom,
        ]);

        const originAnchorMesh = new Mesh(
            mergedGeometry,
        ...
        setComponent(originAnchorEntity, EntityTreeComponent, {
            ...
            if (hitTestResult) {
                if (!hitTestResult.createAnchor) {
                    ...
                    const hitPose =
                        ...
                        hitTestResult.getPose(ReferenceSpace.localFloor);
                    hitPose &&
                        xrFrame
                            ?.createAnchor?.(hitPose.transform, ReferenceSpace.localFloor)
                            ?.then(anchor => {
                                if (!active) {
                                    anchor.delete();
                                    return;
    ...
    useEffect(() => {
        if (
            !xrSession ||
            xrSession.interactionMode !== "world-space" ||
            scenePlacementMode !== "placing"
        )
            return;
