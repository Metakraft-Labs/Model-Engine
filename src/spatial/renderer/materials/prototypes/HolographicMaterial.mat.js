...
import {
    UpdatableCallback,
    UpdatableComponent,
} from "../../../../engine/scene/components/UpdatableComponent";
...
export class HolographicMaterial extends MeshStandardMaterial {
    _uniforms;
    constructor(args) {
        const basicParms = Object.fromEntries(
            Object.keys(args ?? {})
                .filter(k => Object.hasOwn(BasicArgs, k))
                .map(k => [k, args[k]]),
        );
        super(basicParms);
        const uniform = k => new Uniform(args?.[k] ?? DefaultArgs[k].default);
        ...
        addOBCPlugin(this, (shader, renderer) => {
