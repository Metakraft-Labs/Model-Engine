    ...
    update() {
        if (this.sourceLight)
            this.lightDirection.subVectors(
                this.sourceLight.target.position,
                this.sourceLight.position,
            );
        if (this.needsUpdate) {
            this.injectInclude();
            this.updateFrustums();
        ...
        for (let i = 0; i < frustums.length; i++) {
            ...
            light.target.matrix.compose(
                light.target.position,
                light.target.quaternion,
                light.target.scale,
            );
