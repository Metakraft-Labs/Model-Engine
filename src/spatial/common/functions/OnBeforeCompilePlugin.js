...
export function addOBCPlugin(material, plugin) {
  material.onBeforeCompile = plugin
  material.needsUpdate = true
}

/**@deprecated Use removePlugin instead */
export function removeOBCPlugin(material, plugin) {
  if (material.plugins) {
...
function indexOfPlugin(plugin, arr) {
  if (typeof plugin === 'function') {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      else if ((arr[i]).id === plugin.id) return i
    }
  }
...
function sortPluginsByPriority(a, b) {
...
const onBeforeCompile = {
  ...
  set: function (this, plugins) {
    if (plugins === null) {
      ...
      ;(plugin).priority =
        typeof (plugin).priority === 'undefined' ? 1 : (plugin).priority
