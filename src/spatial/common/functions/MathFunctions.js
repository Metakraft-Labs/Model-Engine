...
export const lookAt = (position, _rotation, targetPos) => {
...
// bank
export function pitchFromQuaternion(q) {
    const wx = q.w * q.x;
    const yz = q.y * q.z;
    const x2 = q.x * q.x;
    const y2 = q.y * q.y;
...
export function getPrependicularVector(dir, out) {
...
export function createConeOfVectors(inputVec, outputs, angle) {
    if (outputs.length < 1) {
        return;
    }
...
export const extractRotationAboutAxis = (rot, direction, out) => {
