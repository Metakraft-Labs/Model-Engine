...
export class DualQuaternion {
     ...
     */
    makeFromXRPose(pose) {
        return this.makeFromRotationTranslation(
     ...
     */
    makeFromRotationTranslation(q, t) {
        const qx = q.x;
        const qy = q.y;
        const qz = q.z;
        const qw = q.w;
    ...
    dot(b) {
        return (
            this.real.x * b.real.x +
            this.real.y * b.real.y +
            this.real.z * b.real.z +
            this.real.w * b.real.w
        );
    }
    ...
    sclerp(target, t) {
        ...
        const moment = vd
            .sub(direction.multiplyScalar(pitch * diff.real.w * 0.5))
            .multiplyScalar(invr);
        // Exponential power
        angle *= t;
        pitch *= t;
