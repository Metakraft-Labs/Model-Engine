...
import { Clock, MathUtils, Matrix4, Raycaster, Vector3 } from "three";
import { defineQuery, ECSState, Engine, useEntityContext } from "../../../ecs";
import {
    defineComponent,
    getComponent,
    getMutableComponent,
...
import { getState, matches, useImmediateEffect } from "../../../hyperflux";
...
export const coneDebugHelpers = [];

const window = "window" in globalThis ? globalThis.window : {};
...
export const FollowCameraComponent = defineComponent({
    ...
    onInit: _entity => {
        ...
        //     data.cameraFarClip
        //   )
        // } else if ((camera.camera).fov) {
        //   ;(camera.camera).fov = data.fov ?? 50
        // }
        ...
        return {
            ...
            lastZoomStartDistance:
                (cameraSettings.minCameraDistance + cameraSettings.minCameraDistance) / 2,
        };
    },
    ...
    onSet: (entity, component, json) => {
        ...
        if (typeof json.firstPersonOffset !== "undefined")
            component.firstPersonOffset.set(json.firstPersonOffset);
        if (typeof json.thirdPersonOffset !== "undefined")
            component.thirdPersonOffset.set(json.thirdPersonOffset);
        ...
        if (matches.arrayOf(matches.string).test(json.allowedModes))
            component.allowedModes.set(json.allowedModes);
        if (typeof json.distance !== "undefined") component.distance.set(json.distance);
        if (typeof json.targetDistance !== "undefined")
            component.targetDistance.set(json.targetDistance);
...
const computeCameraFollow = (cameraEntity, referenceEntity) => {
    ...
    targetPosition
        ...
        .applyQuaternion(
            TransformComponent.getWorldRotation(referenceEntity, targetTransform.rotation),
        )
        .add(TransformComponent.getWorldPosition(referenceEntity, new Vector3()));

    const targetPositionAlpha = smootheLerpAlpha(
        follow.targetPositionSmoothness,
    ...
    if (follow.mode === FollowCameraMode.FirstPerson) {
        ...
        follow.effectiveMaxDistance = Math.min(
            obstacleDistance * 0.8,
            follow.thirdPersonMaxDistance,
        );
        follow.effectiveMinDistance = Math.min(
            follow.thirdPersonMinDistance,
            follow.effectiveMaxDistance,
        );
    ...
    if (follow.mode === FollowCameraMode.FirstPerson) {
        ...
        if (triggerZoomShift) {
            ...
            if (
                ...
                follow.targetDistance <
                    follow.effectiveMinDistance - follow.effectiveMaxDistance * 0.05 &&
                Math.abs(follow.lastZoomStartDistance - follow.effectiveMinDistance) <
                    follow.effectiveMaxDistance * 0.05
            ) {
                setTargetCameraRotation(cameraEntity, 0, follow.theta);
                ...
                follow.targetDistance >
                    follow.effectiveMaxDistance + follow.effectiveMaxDistance * 0.02 &&
                Math.abs(follow.lastZoomStartDistance - follow.effectiveMaxDistance) <
                    follow.effectiveMaxDistance * 0.02
            ) {
        ...
        if (triggerZoomShift) {
            ...
            if (
                ...
                Math.abs(follow.lastZoomStartDistance - follow.effectiveMaxDistance) <
                    0.05 * follow.effectiveMaxDistance
            ) {
                setTargetCameraRotation(cameraEntity, 0, follow.theta);
                followState.mode.set(FollowCameraMode.ThirdPerson);
            }
...
const updateCameraTargetRotation = cameraEntity => {
    ...
    if (
        Math.abs(target.phi - followCamera.phi) < epsilon &&
        Math.abs(target.theta - followCamera.theta) < epsilon
    ) {
...
const cameraLayerQuery = defineQuery([
    VisibleComponent,
    ObjectLayerComponents[ObjectLayers.Camera],
    MeshComponent,
]);

const getMaxCamDistance = (cameraEntity, target) => {
    const followCamera = getComponent(cameraEntity, FollowCameraComponent);
    ...
    if (
        ...
        camRayCastClock.getElapsedTime() < raycastProps.rayFrequency
    ) {
