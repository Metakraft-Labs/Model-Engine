    ...
    onSet: (entity, component, json) => {
        ...
        if (currentParentEntity && currentParentEntity !== json.parentEntity) {
            if (entityExists(currentParentEntity)) {
                const oldParent = getOptionalMutableComponent(
                    currentParentEntity,
                    EntityTreeComponent,
                );
                if (oldParent) {
                    const parentChildIndex = oldParent.children.value.findIndex(
                        child => child === entity,
                    );
                    const children = oldParent.children.get(NO_PROXY);
                    oldParent.children.set([
                        ...children.slice(0, parentChildIndex),
                        ...children.slice(parentChildIndex + 1),
                    ]);
                }
            }
        }
        ...
        if (parentEntity && entityExists(parentEntity)) {
            ...
            const isDifferentIndex =
                typeof json.childIndex === "number" ? prevChildIndex !== json.childIndex : false;
 ...
 */
export function destroyEntityTree(entity) {
 ...
 */
export function removeFromEntityTree(entity) {
    const children = getComponent(entity, EntityTreeComponent).children.slice();
    for (const child of children) {
 ...
 * @param tree
 */
export function removeEntityNodeRecursively(entity) {
 ...
 */
export function traverseEntityNode(entity, cb, index = 0) {
...
export function traverseEntityNodeChildFirst(entity, cb, index = 0) {
 ...
 */
export function traverseEntityNodeParent(entity, cb) {
    const entityTreeNode = getOptionalComponent(entity, EntityTreeComponent);
 ...
 * @returns
 */
export function getAncestorWithComponent(entity, component, closest = true, includeSelf = true) {
 ...
 */
export function findIndexOfEntityNode(arr, obj) {
...
export function isDeepChildOf(child, parent) {
...
export function getNestedChildren(entity, predicate) {
    const children = [];
    iterateEntityNode(
        entity,
        child => {
 ...
 * @returns
 */
export function useTreeQuery(entity) {
 ...
 * @returns
 */
export function useAncestorWithComponent(entity, component) {
    const result = useHookstate(() => getAncestorWithComponent(entity, component));

    useImmediateEffect(() => {
        let unmounted = false;
        const ParentSubReactor = props => {
            const tree = useOptionalComponent(props.entity, EntityTreeComponent);
            ...
            return (
                <ParentSubReactor key={tree.parentEntity.value} entity={tree.parentEntity.value} />
            );
        };
 ...
 *
 */
export function useChildWithComponent(rootEntity, component) {
    const result = useHookstate(UndefinedEntity);

    useLayoutEffect(() => {
        let unmounted = false;
        const ChildSubReactor = props => {
            const tree = useOptionalComponent(props.entity, EntityTreeComponent);
            const matchesQuery = !!useOptionalComponent(props.entity, component)?.value;
...
export function useChildrenWithComponent(rootEntity, component) {
    const children = useHookstate([]);
...
// export function TreeQueryReactor (props: { Components: QueryComponents; ChildEntityReactor: FC; props? }) {

// }

export function haveCommonAncestor(entity1, entity2) {
    const entity1Ancestors = [];
    const entity2Ancestors = [];
...
export function findCommonAncestors(objects = [], target = []) {
...
export function isAncestor(parent, potentialChild, includeSelf = false) {
...
export function traverseEarlyOut(entity, cb) {
...
export const filterParentEntities = (
    ...
    const traverseParentOnly = entity => {
        if (!entity) return;
