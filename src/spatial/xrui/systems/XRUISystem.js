...
const redirectDOMEvent = evt => {
    for (const entity of visibleInteractableXRUIQuery()) {
        ...
        if (inputSource.intersections.length && inputSource.intersections[0].entity !== entity)
            continue; // only handle events for the first intersection
...
const updateControllerRayInteraction = (entity, xruiEntities) => {
    ...
    for (const entity of xruiEntities) {
        ...
        if (layerHit && (!hit || layerHit.intersection.distance < hit.intersection.distance))
            hit = layerHit;
    }
...
const updateClickEventsForController = entity => {
    const pointerComponentState = getMutableComponent(entity, PointerComponent);
...
const execute = () => {
    ...
    for (const [pointerSource, entity] of PointerComponent.pointers) {
        if (
            !inputSourceEntities.find(
                entity => getComponent(entity, InputSourceComponent).source === pointerSource,
            )
        ) {
            removeEntity(entity);
        }
