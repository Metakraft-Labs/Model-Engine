...
import { none } from "../../hyperflux/StateFunctions";

/** @todo refactor this and WidgetAppState into WidgetState */
export const RegisteredWidgets = new Map();
...
export const WidgetAppService = {
    closeWidgets: () => {
        ...
        const widgets = Object.entries(widgetMutableState.widgets.value).map(
            ([id, widgetMutableState]) => ({
                id,
                ...widgetMutableState,
    ...
    setWidgetVisibility: (widgetName, visibility) => {
        const widgetMutableState = getMutableState(WidgetAppState);
        const widgets = Object.entries(widgetMutableState.widgets.value).map(
            ([id, widgetMutableState]) => ({
                id,
        ...
        currentWidget &&
            dispatchAction(
                WidgetAppActions.showWidget({ id: currentWidget.id, shown: visibility }),
            );
    },
};
...
export class WidgetAppActions {
    static showWidgetMenu = defineAction({
        type: "xre.xrui.WidgetAppActions.SHOW_WIDGET_MENU",
        shown: matches.boolean,
        handedness: matches.string.optional(),
    });

    static registerWidget = defineAction({
        type: "xre.xrui.WidgetAppActions.REGISTER_WIDGET",
        id: matches.string,
    });

    static unregisterWidget = defineAction({
        type: "xre.xrui.WidgetAppActions.UNREGISTER_WIDGET",
        id: matches.string,
    });

    static enableWidget = defineAction({
        type: "xre.xrui.WidgetAppActions.ENABLE_WIDGET",
        id: matches.string,
        enabled: matches.boolean,
    });

    static showWidget = defineAction({
        type: "xre.xrui.WidgetAppActions.SHOW_WIDGET",
        id: matches.string,
        shown: matches.boolean,
