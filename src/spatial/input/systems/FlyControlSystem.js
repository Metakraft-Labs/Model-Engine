...
const onSecondaryClick = viewerEntity => {
...
const onSecondaryReleased = viewerEntity => {
    ...
    editorCameraCenter.add(
        directionToCenter.set(0, 0, -centerLength).applyQuaternion(transform.rotation),
    );
...
const execute = () => {
    ...
    for (const entity of flyControlQuery()) {
        ...
        const allowRotationInX =
            newCamUpY > 0 &&
            ((newCamForwardY < extrema && newCamForwardY > -extrema) || newCamUpY > camUpY);
        ...
        transform.position.y +=
            upwardMovement * deltaSeconds * flyControlComponent.moveSpeed * boostSpeed;
    }
};
