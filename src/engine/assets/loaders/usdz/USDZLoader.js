...
class USDZLoader extends Loader {
    ...
    parse(buffer, onLoad) {
        ...
        function buildGeometry(data) {
            ...
            if (
                "int[] faceVertexIndices" in data &&
                typeof data["int[] faceVertexIndices"] === "string"
            ) {
            ...
            if (
                "texCoord2f[] primvars:st" in data &&
        ...
        function buildMaterial(data) {
            ...
            if (data !== undefined) {
                const surfaceRegexp =
                    /def Shader "(PreviewSurface|surfaceShader|Principled_BSDF|[^"]+_preview)"/;
                ...
                if (key !== undefined) {
                    ...
                    for (const colorType of acceptedColorTypes) {
                        ...
                        if (`${colorType} inputs:diffuseColor` in surface) {
                            const color = surface[`${colorType} inputs:diffuseColor`].replace(
                                /[()]*/g,
                                "",
                            );
                    ...
                    for (const colorType of acceptedColorTypes) {
                        if (`${colorType} inputs:emissiveColor` in surface) {
                            const color = surface[`${colorType} inputs:emissiveColor`].replace(
                                /[()]*/g,
                                "",
                            );
        ...
        function buildTexture(data) {
            if ("asset inputs:file" in data) {
                const path = data["asset inputs:file"]
                    .replace(/@*/g, "")
                    .replace(/(\.)?[\\\/]+/g, "/")
                    .replace(/^\//, "");
                if (!(path in assets)) {
                ...
                if (textureCache[path]) {
                    pending.push(
                        new Promise(resolve =>
                            textureCache[path].then(_texture => {
                                if (!texture) resolve();
        ...
        function buildMesh(data) {
            const geometry = buildGeometry(
                "point3f[] points" in data ? data : findMeshGeometry(data),
            );
            ...
            const meshPlugins = plugins.filter(plugin => !!plugin.buildMesh);
        ...
        function buildPointInstancer(registry, frontier, [path, context, name]) {
            ...
            if (
                "rel prototypes" in nameContext &&
                ["object", "string"].includes(typeof nameContext["rel prototypes"])
            ) {
                const prototypePaths =
                    typeof nameContext["rel prototypes"] === "object"
            ...
            if (typeof nameContext["int[] protoIndices"] === "string") {
                const protoIndices = JSON.parse(
                    nameContext["int[] protoIndices"].replace(/[()]*/g, ""),
                );
                ...
                if (typeof nameContext["point3f[] positions"] === "string") {
                    ...
                    const positionArr = JSON.parse(
                        nameContext["point3f[] positions"].replace(/[()]*/g, ""),
                    );
                ...
                if (typeof nameContext["quath[] orientations"] === "string") {
                    ...
                    const quatArr = JSON.parse(
                        nameContext["quath[] orientations"].replace(/[()]*/g, ""),
                    );
                    for (let i = 0; i < instanceCount; i++) {
                        const idx = i * 4;
                ...
                if (typeof nameContext["float3[] scales"] === "string") {
                    ...
                    const scaleArr = JSON.parse(
                        nameContext["float3[] scales"].replace(/[()]*/g, ""),
                    );
                    for (let i = 0; i < instanceCount; i++) {
                        const idx = i * 3;
                ...
                const instancedMeshes = Object.entries(instanceTable).map(
                    ([index, { matrices, baseMesh }]) => {
        ...
        while (frontier.length > 0) {
            ...
            if (registryRegex.test(name)) {
                ...
                frontier.push(
                    ...Object.keys(nameContext).map(field => [registryPath, nameContext, field]),
                );
                registry[registryPath] = registry[path];
            }
            if (name.startsWith("def PointInstancer")) {
                const pointInstancer = buildPointInstancer(registry, frontier, [
                    path,
                    context,
                    name,
                ]);
            ...
            if (name.startsWith("def Xform")) {
                ...
                const xformPlugins = plugins.filter(plugin => !!plugin.buildXform);
                ...
                keys.map(key => {
                    if (fieldRegex.test(key)) {
                        ...
                        if (registryRegex.test(key)) {
                            ...
                            frontier.push(
                                ...Object.keys(keyContext).map(field => [
                                    keyPath,
                                    keyContext,
                                    field,
                                ]),
