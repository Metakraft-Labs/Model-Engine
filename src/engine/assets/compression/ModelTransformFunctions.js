...
function removeUVsOnUntexturedMeshes(document) {
    document
        .getRoot()
        .listMeshes()
        .map(mesh => {
    ...
    toSplit.map(node => {
        ...
        mesh.listPrimitives().map((prim, primIdx) => {
            if (!primMeshes.has(prim)) {
                primMeshes.set(
                    prim,
                    document.createMesh(mesh.getName() + "-" + primIdx).addPrimitive(prim),
                );
            } else {
                console.log("found cached prim");
            }
            const nuNode = document
                .createNode(node.getName() + "-" + primIdx)
                .setMesh(primMeshes.get(prim));
...
const myInstance = async document => {
    const root = document.getRoot();
    const scene = root.listScenes()[0];
...
function unInstanceSingletons(document) {
    const root = document.getRoot();
    root.listNodes()
        .filter(
            node =>
                node.getExtension("EXT_mesh_gpu_instancing")?.listAttributes()?.[0].getCount() ===
                1,
        )
        .map(node => {
...
export async function combineMaterials(document) {
    const root = document.getRoot();
    const cache = [];
...
export async function combineMeshes(document) {
    const root = document.getRoot();
        ...
        prims.map(prim => {
            prim.listSemantics().map(key => {
                ...
                if (!nuAttrib) {
                    ...
                    nuAttrib.setArray(
                        BufferUtils.concat([
                            Uint8Array.from(nuAttrib.getArray()),
                            Uint8Array.from(accessor.getArray()),
                        ]),
                    );
                }
            });
    ...
    nuPrims.map(nuPrim => {
        root.listScenes()[0].addChild(
            document.createNode().setMesh(document.createMesh().addPrimitive(nuPrim)),
        );
    });
}
...
export async function transformModel(args, onMetadata = (key, data) => {}) {
    const parms = args;
    ...
    const resourcePath = pathJoin(
        LoaderUtils.extractUrlBase(args.src),
        args.resourceUri || resourceName + "_resources",
    );

    const toValidFilename = name => {
        const result = name.replace(/[\s]/g, "-");
        return result;
    };
    let pathIndex = 0;
    const toPath = (element, index) => {
    ...
    const fileUploadPath = fUploadPath => {
        ...
        const [_, projectName, fileName] =
            ...
            pathCheck.exec(pathJoin(LoaderUtils.extractUrlBase(args.src), fUploadPath));
  ...
  */
    ...
    if (args.meshoptCompression.enabled) {
        const meshoptCompression = document
    ...
    const eeMaterialExtension = root
        .listExtensionsUsed()
        .find(ext => ext.extensionName === "EE_material");
    ...
    if (parms.textureFormat !== "default") {
        ...
        for (const texture of textures) {
            ...
            if (!oldSize) continue;
            const resourceId = texture.getExtension(
                EEResourceIDExtension.EXTENSION_NAME,
            )?.resourceId;
            ...
            if (oldSize.reduce((x, y) => Math.max(x, y)) > mergedParms.maxTextureSize) {
                ...
                const quality =
                    mergedParms.textureCompressionType === "uastc"
                        ? mergedParms.uastcLevel
                        : mergedParms.compLevel;
            ...
            if (mergedParms.textureFormat === "ktx2" && texture.getMimeType() !== "image/ktx2") {
                ...
                const clampedData = new Uint8ClampedArray(texturePixels.data);
