...
const getLoaderForResourceType = resourceType => {
    switch (resourceType) {
    ...
    if (!resources[url].value) {
        ...
        // If asset already exists clone it to share GPU memory
            ...
            ResourceState.debugLog(
                `ResourceManager:load cloning already loaded asset: ${url} for entity: ${entity}`,
            );
            return;
        }
    }
    ...
    AssetLoader.loadAsset(
        ...
        response => {
            if (!resource || !resource.value) {
                console.warn(
                    `ResourceManager:load Resource removed before load finished: ${url} for entity: ${entity}`,
                );
                return;
            }
            ...
            ResourceState.debugLog(
                `ResourceManager:load Loaded resource: ${url} for entity: ${entity}`,
            );
            ...
            if (pending[url]) {
                for (const pendingLoad of pending[url]) {
                    if (!cloneAsset(response, pendingLoad))
                        console.warn(
                            `ResourceManager:load unable to clone asset for pending response: ${url}`,
                        );
                    else
                        ResourceState.debugLog(
                            `ResourceManager:load cloning pending asset: ${url}`,
                        );
                }
                pending[url].clear();
            }
        ...
        error => {
            console.warn(
                `ResourceManager:load error loading ${resourceType} at url ${url} for entity ${entity}`,
                error,
            );
            if (resource && resource.value) {
...
const updateResource = url => {
    ...
    if (!resource.value) {
        console.warn(
            "resourceLoaderFunctions:updateResource No resource found to update for url: " + url,
        );
        return;
    }
    ...
    if (!onLoads) {
        ResourceState.debugLog(
            "resourceLoaderFunctions:updateResource No callbacks found to update for url: " + url,
        );
        return;
    }
    ...
    for (const [uuid, loadObj] of Object.entries(onLoads)) {
        loadResource(
            ...
            error => {
                console.error(
                    "resourceLoaderFunctions:updateResource error updating resource for url: " +
                        url,
                    error,
                );
            },
