...
function getImageHash(image, size = 8) {
...
function stageHash(pHash, bits = 256) {
    const binaryHash = (typeof pHash === "number" ? pHash : parseInt(pHash, 16))
        .toString(2)
        .padStart(bits, "0");
    return binaryHash;
}
...
export default class BasisuExporterExtension extends ExporterExtension {
    ...
    writeTexture(_texture, textureDef) {
        ...
        writer.pending.push(
            new Promise(resolve => {
                createReadableTexture(_texture, { canvas: true, flipY: true }).then(texture => {
                    ...
                    similarImages.length &&
                        ((imageDef.uri = this.imageIdCache[similarImages[0]]) || resolve(null));
                    !similarImages.length &&
                        ktx2Encoder
                            .encode(imgData, {
                                qualityLevel: 256,
                            ...
                            .then(async arrayBuffer => {
                                if (writer.options.embedImages) {
                                    ...
                                    const bufferViewIdx =
                                        writer.json.bufferViews.push(bufferViewDef) - 1;
                                    ...
                                    const basePath = writer.options.relativePath.replace(
                                        /^\/*assets\//,
                                        "",
                                    );
                                    const baseURI = basePath.includes("/")
