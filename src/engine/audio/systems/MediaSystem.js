...
import { StandardCallbacks, setCallback } from "../../../spatial/common/CallbackComponent";
...
import {
    VideoComponent,
    VideoTexturePriorityQueueState,
...
export class AudioEffectPlayer {
    ...
    play = async (sound, volumeMultiplier = getState(AudioState).notificationVolume) => {
...
const execute = () => {
    ...
    for (const entity of VideoComponent.uniqueVideoEntities) {
        ...
        const videoTexture = getComponent(videoMeshEntity, MeshComponent).material.map;
...
const reactor = () => {
    useEffect(() => {
        ...
        return () => {
            for (const sound of Object.values(AudioEffectPlayer.SOUNDS))
                delete AudioEffectPlayer.instance.bufferMap[sound];
        };
    }, []);
