        ...
        const node = makeFlowNodeDefinition({
            ...
            triggered: ({ read, commit }) => {
                ...
                for (const [input, type] of inputs) {
                    actionInput[input] = NodetoEnginetype(read(input), type);
                }
                dispatchAction(ActionDefinitions[type](actionInput));
                commit("flow");
            },
        });
                    ...
                    execute: () => {
                        ...
                        for (let i = 0; i < tempQueue.length; i++) {
                            ...
                            for (const [output, type] of Object.entries(outputSockets)) {
                                write(output, NodetoEnginetype(currAction[output], type));
                            }
                            commit("flow");
                        }
