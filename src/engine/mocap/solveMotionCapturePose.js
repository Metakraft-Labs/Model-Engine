...
import { Vector3_Right, Vector3_Up } from "../../spatial/common/constants/MathConstants";
import { NameComponent } from "../../spatial/common/NameComponent";
import { addObjectToGroup, GroupComponent } from "../../spatial/renderer/components/GroupComponent";
 ...
 */
export const calculateGroundedFeet = newLandmarks => {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const footMoving =
            Math.abs(footVerticalPositions[i] - footAverages[i]) > footAverageDifferenceThreshold;
        //compare foot level for right and left foot
        const footLevel =
            Math.abs(footVerticalPositions[i] - footVerticalPositions[i == 0 ? 1 : 0]) <
            footLevelDifferenceThreshold;
...
const drawMocapDebug = label => {
    if (!PoseLandmarker.POSE_CONNECTIONS) return () => {};

    const debugEntities = {};
    let lineSegmentEntity = null;

    const positionLineSegment = new LineSegments(
        new BufferGeometry(),
        new LineBasicMaterial({ vertexColors: true }),
    );
    positionLineSegment.material.linewidth = 4;
    const posAttr = new BufferAttribute(
        new Float32Array(PoseLandmarker.POSE_CONNECTIONS.length * 2 * 3).fill(0),
        3,
    );
    positionLineSegment.geometry.setAttribute("position", posAttr);
    const colAttr = new BufferAttribute(
        new Float32Array(PoseLandmarker.POSE_CONNECTIONS.length * 2 * 4).fill(1),
        4,
    );
    positionLineSegment.geometry.setAttribute("color", colAttr);

    return (landmarks, debugEnabled) => {
        if (!debugEnabled) {
        ...
        for (const [key, value] of Object.entries(landmarks)) {
            ...
            const mesh = getComponent(entity, GroupComponent)[0];
            ...
            getComponent(entity, TransformComponent).matrix.setPosition(
                value.x,
                lowestWorldY - value.y,
                value.z,
            );
        }
        ...
        for (let i = 0; i < PoseLandmarker.POSE_CONNECTIONS.length * 2; i += 2) {
            ...
            const firstPoint = getComponent(debugEntities[start], GroupComponent)[0];
            const secondPoint = getComponent(debugEntities[end], GroupComponent)[0];
            ...
            const confident =
                firstPoint.material.color.g > threshhold &&
                secondPoint.material.color.g > threshhold;
...
const shouldEstimateLowerBody = (landmarks, threshold = 0.5) => {
...
export function solveMotionCapturePose(entity, newLandmarks, newScreenlandmarks) {
    const keyframeInterpolation = (newLandmarks, prevLandmarks, alphaMultiplier) => {
        if (!prevLandmarks.length) return newLandmarks;
        const filteredLandmarks = [];
        const lowPassLandmarks = [];
        for (let i = 0; i < newLandmarks.length; i++) {
            ...
            lowPassLandmarks[i] = {
                visibility: MathUtils.lerp(
                    prevLandmarks[i].visibility,
                    newLandmarks[i].visibility,
                    alpha,
                ),
                x: MathUtils.lerp(
                    prevLandmarks[i].x,
...
export const solveSpine = (entity, lowestWorldY, landmarks, trackingLowerBody) => {
    const rig = getComponent(entity, AvatarRigComponent);
    ...
    const shoulderLeft = new Vector3(
        -rightShoulder.x,
        lowestWorldY - rightShoulder.y,
        -rightShoulder.z,
    );
    const shoulderRight = new Vector3(
        -leftShoulder.x,
        lowestWorldY - leftShoulder.y,
        -leftShoulder.z,
    );

    const shoulderCenter = new Vector3().copy(shoulderLeft).add(shoulderRight).multiplyScalar(0.5);
    hipToShoulderQuaternion.setFromUnitVectors(
        Vector3_Up,
...
export const solveLimb = (
    ...
    if (
        minimumVisibility > -1 &&
        (start.visibility + mid.visibility + end.visibility) / 3 < minimumVisibility
    )
        return;
    ...
    const midQuaternion = new Quaternion().setFromUnitVectors(
        axis,
    ...
    const midLocal = new Quaternion()
        .copy(midQuaternion)
        .premultiply(startQuaternion.clone().invert());
...
export const solveHand = (
    ...
    ref1,
    ref2,
    parentTargetBoneName,
    extentTargetBoneName,
) => {
    ...
    directionVector
        .addVectors(ref1Point, ref2Point)
        .multiplyScalar(0.5)
        .sub(startPoint)
        .normalize(); // Calculate direction between wrist and center of tip of hand
...
const rotate90degreesAroundXAxis = new Quaternion().setFromAxisAngle(
    new Vector3(1, 0, 0),
    -Math.PI / 2,
);

export const solveHead = (entity, leftEar, rightEar, nose) => {
...
const getQuaternionFromPointsAlongPlane = (a, b, planeRestraint, target, invert = false) => {
    plane.setFromCoplanarPoints(invert ? b : a, invert ? a : b, planeRestraint);
    const orthogonalVector = plane.normal;
    directionVector.subVectors(a, b).normalize();
    thirdVector.crossVectors(
        orthogonalVector,
        invert ? directionVector.reflect(new Vector3(0, 1, 0)) : directionVector,
    );
