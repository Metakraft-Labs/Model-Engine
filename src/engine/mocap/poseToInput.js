...
import { getState, none } from "../../hyperflux";
...
export const evaluatePose = entity => {
    const rig = getComponent(entity, AvatarRigComponent).normalizedRig;
    if (!rig) return;

    const deltaSeconds = getState(ECSState).deltaSeconds;

    if (!hasComponent(entity, MotionCapturePoseComponent))
        setComponent(entity, MotionCapturePoseComponent);
    ...
    const getLegsSeatedChange = toPose => {
