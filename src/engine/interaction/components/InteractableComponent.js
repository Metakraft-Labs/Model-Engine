...
export const updateInteractableUI = entity => {
    ...
    if (hasVisibleComponent && boundingBox) {
        ...
        xruiTransform.position.y = MathUtils.lerp(
    ...
    if (inCameraFrustum) {
        if (interactable.uiVisibilityOverride === XRUIVisibilityOverride.none) {
            ...
            if (
                !activateUI &&
                (interactable.uiActivationType === XRUIActivationType.hover ||
                    interactable.clickInteract)
            ) {
                //hover
...
const addInteractableUI = entity => {
    ...
    setComponent(uiEntity, EntityTreeComponent, {
...
export const InteractableComponent = defineComponent({
    ...
    onSet: (_entity, component, json) => {
        ...
        if (
            typeof json.uiActivationType === "number" &&
            component.uiActivationType.value !== json.uiActivationType
        )
            component.uiActivationType.set(json.uiActivationType);
        if (
            typeof json.clickInteract === "boolean" &&
            component.clickInteract.value !== json.clickInteract
        )
            component.clickInteract.set(json.clickInteract);
        if (
            typeof json.uiInteractable === "boolean" &&
            component.uiInteractable.value !== json.uiInteractable
        )
    ...
    reactor: () => {
        ...
        InputComponent.useExecuteWithInput(
            () => {
                ...
                if (!interactableComponent.clickInteract.value && buttons.PrimaryClick?.pressed)
                    return;
                if (
