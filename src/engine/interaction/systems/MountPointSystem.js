  ...
  */
    ...
    if (mocapInputSource) {
        if (mocapInputSource.sitting?.begun)
            MountPointComponent.mountEntity(
                selfAvatarEntity,
                getState(InteractableState).available[0],
            );
    ...
    for (const entity of sittingIdleQuery()) {
        ...
        const mountTransform = getComponent(
            getComponent(entity, SittingComponent).mountPointEntity,
            TransformComponent,
        );
        ...
        setComponent(entity, TransformComponent, {
            rotation: mountTransform.rotation,
            position: vec3_0,
        });
