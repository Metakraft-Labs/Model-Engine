...
export const setIkFootTarget = (userID, delta) => {
    const selfAvatarEntity = AvatarComponent.getUserAvatarEntity(userID);

    const leftFootEntity = UUIDComponent.getEntityByUUID(userID + ikTargets.leftFoot);
    const rightFootEntity = UUIDComponent.getEntityByUUID(userID + ikTargets.rightFoot);
    ...
    for (const [key, foot] of Object.entries(feet)) {
        ...
        walkDirection
            .subVectors(playerRigidbody.position, playerRigidbody.previousPosition)
            .setY(0);
        const playerDisplacement = walkDirection.length();

        //get distance from the player
        const ikDistanceSqFromPlayer = ikTargetToPlayer
            .subVectors(ikTransform.position, footOffset)
            .setY(0)
            .lengthSq();
        ...
        if (ikDistanceSqFromPlayer > stepThresholdSq) {
            nextStep[key].position
                ...
                .add(
                    walkDirection
                        .normalize()
                        .multiplyScalar((stepDistance * playerDisplacement) / delta),
                );

            nextStep[key].rotation
                .identity()
                .setFromUnitVectors(Vector3_Up, walkDirection.normalize());
        }
