...
import { Q_Y_180 } from "../../../spatial/common/constants/MathConstants";
...
} from "../../../spatial/xr/XRComponents";
import { ReferenceSpace, XRState } from "../../../spatial/xr/XRState";
 ...
 * @param entity
 */
const applyHandPose = (inputSource, entity) => {
    ...
    const component =
        inputSource.handedness === "right" ? XRRightHandComponent : XRLeftHandComponent;
    ...
    for (let i = 1; i < 25; i++) {
        ...
        if (joint === "thumb-metacarpal")
            quat.multiply(
                inputSource.handedness == "right" ? rightControllerOffset : leftControllerOffset,
            );
 ...
 */
export const applyInputSourcePoseToIKTargets = userID => {
    const xrFrame = getState(XRState).xrFrame;
    ...
    for (const inputSourceEntity of inputSourceQuery()) {
        ...
        if (entity) {
            ...
            const hand = inputSourceComponent.source.hand;
            ...
            if (hand && xrFrame.fillPoses && xrFrame.getJointPose) {
                ...
                if (inputSourceComponent.source.gripSpace) {
                    const pose = xrFrame.getPose(
                        inputSourceComponent.source.gripSpace,
                        referenceSpace,
                    );
                    if (pose) {
                        ikTransform.position.copy(pose.transform.position);
                        // .sub(localClientTransform.position)
                        // .multiplyScalar(inverseWorldScale)
                        // .add(localClientTransform.position)
                        ikTransform.rotation
                            .copy(pose.transform.orientation)
                            .multiply(
                                handedness === "right"
                                    ? rightControllerOffset
                                    : leftControllerOffset,
