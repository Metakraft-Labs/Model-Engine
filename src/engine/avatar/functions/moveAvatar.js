...
import { dispatchAction, getState } from "../../../hyperflux";
...
import {
    ...
    updateWorldOrigin,
} from "../../../spatial/transform/updateWorldOrigin";
...
export function moveAvatar(entity, additionalMovement) {
    ...
    if (isCameraAttachedToAvatar) {
        ...
        viewerPose &&
            viewerMovement
                .copy(viewerPose.transform.position)
    ...
    if (groundHits.length) {
        ...
        controller.isInAir =
            hit.distance > avatarGroundRaycastDistanceOffset + controllerOffset * 10; // todo - 10 is way too big, should be 1, but this makes you fall down stairs
...
export const updateReferenceSpaceFromAvatarMovement = (entity, movement) => {
    const originTransform = getComponent(Engine.instance.localFloorEntity, TransformComponent);
...
export const applyAutopilotInput = entity => {
    ...
    const legSpeed = controller.isWalking
        ? avatarMovementSettings.walkSpeed
        : avatarMovementSettings.runSpeed;
...
export const applyGamepadInput = entity => {
    if (!entity) return;
...
const applyVerticalVelocity = (controller, avatarMovementSettings) => {
    if (!controller.isInAir) {
        controller.verticalVelocity = 0;
        if (controller.gamepadJumpActive) {
            if (!controller.isJumping) {
                // Formula: takeoffVelocity = sqrt(2 * jumpHeight * gravity)
                controller.verticalVelocity = Math.sqrt(
                    2 * avatarMovementSettings.jumpHeight * 9.81,
                );
 ...
 * @param rotation
 */
export const spinMatrixWithQuaternion = (matrix, rotation) => {
    rotateMatrixAboutPoint(
        matrix,
        vec3.set(matrix.elements[12], matrix.elements[13], matrix.elements[14]),
        rotation,
    );
};
 ...
 * @param rotation
 */
export const rotateMatrixAboutPoint = (matrix, point, rotation) => {
 ...
 * @param rotation
 */
export const translateAndRotateAvatar = (entity, translation, rotation) => {
    const rigidBody = getComponent(entity, RigidBodyComponent);
    ...
    if (isCameraAttachedToAvatar) {
        ...
        originRelativeToAvatarMatrix
            .copy(avatarTransform.matrix)
            .invert()
            .multiply(originTransform.matrix);
...
export const updateLocalAvatarPosition = entity => {
    const world = Physics.getWorld(entity);
    if (!world) return;
...
const _updateLocalAvatarRotationAttachedMode = entity => {
    const rigidbody = getComponent(entity, RigidBodyComponent);
    ...
    const viewerForward = new Vector3(0, 0, 1).applyQuaternion(viewerQuat).setY(0);
    const rigidbodyForward = new Vector3(0, 0, -1)
        .applyQuaternion(rigidbody.targetKinematicRotation)
        .setY(0);
...
export const updateLocalAvatarRotation = entity => {
 ...
 * @param newPosition
 */
export const teleportAvatar = (entity, targetPosition, force = false) => {
    ...
    if (raycastHit || force) {
        const transform = getComponent(entity, TransformComponent);
        const rigidbody = getComponent(entity, RigidBodyComponent);
        const newPosition = raycastHit ? raycastHit.position : targetPosition;
        ...
        if (isCameraAttachedToAvatar)
            updateReferenceSpaceFromAvatarMovement(
                entity,
                new Vector3().subVectors(newPosition, transform.position),
            );
    } else {
...
const _slerpBodyTowardsVelocity = (entity, alpha) => {
    const rigidbody = getComponent(entity, RigidBodyComponent);
    if (!rigidbody) return;
