...
import { AnimationClip, LoopOnce, LoopRepeat, Vector3 } from "three";
...
import { defineActionQueue, getState } from "../../../hyperflux";
...
export const getAnimationAction = (name, mixer, animations) => {
...
export const updateAnimationGraph = avatarEntities => {
    for (const newAnimation of animationQueue()) {
        ...
        playAvatarAnimationFromMixamo(
            targetEntity,
            animationAsset.scene,
            newAnimation?.loop,
    ...
    for (const entity of avatarEntities) {
        ...
        if (currentAction.value) {
            ...
            if (
                ...
                locomotionBlend.set(
                    Math.max(locomotionBlend.value - deltaSeconds * currentActionBlendSpeed, 0),
                );
                ...
                locomotionBlend.set(
                    Math.min(locomotionBlend.value + deltaSeconds * currentActionBlendSpeed, 1),
                );
            }
        }
    }
...
export const playAvatarAnimationFromMixamo = (entity, animationsScene, loop, clipName) => {
    ...
    if (!retargetedAnimation) {
        retargetedAnimation = bindAnimationClipFromMixamo(
            clipName
                ? animationsScene.animations.find(clip => clip.name == clipName) ??
...
export const getRootSpeed = clip => {
