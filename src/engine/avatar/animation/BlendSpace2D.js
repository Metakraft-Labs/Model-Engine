...
export class BlendSpace2D {
     ...
     */
    addNode(action, position, data = null) {
    ...
    _zeroOtherNodesWeight(currentNodes) {
        this._flatNodeList
            .filter(x => !currentNodes.includes(x))
            .forEach(x => (x.action.weight = 0));
    }

    update() {
        // bi-linear interpolation
        const normalizedValue = this._getNormalizedValue();
        const gridIndex = this._tempVec1.set(
            Math.floor(normalizedValue.x),
            Math.floor(normalizedValue.y),
        );
