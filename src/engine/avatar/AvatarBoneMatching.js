...
const _findHips = root => {
    let hips;
    Object3DUtils.traverse(root, bone => {
    ...
    const shoulderBones = tailBones
        .map(tailBone => {
            const shoulderBone = _findClosestParentBone(
                tailBone,
                bone =>
                    /shoulder|clavicle/i.test(bone.name) &&
                    regexp.test(bone.name.replace(/shoulder|clavicle/gi, "")),
            );
            if (shoulderBone) {
        ...
        .filter(
            bone =>
                ...
                _findClosestParentBone(
                    bone,
                    bone => bone === parent.Left_wrist || bone === parent.Right_wrist,
                ),
        )
        .sort((a, b) => {
...
function findHandBones(handBone) {
    let pinky1,
        pinky2,
        pinky3,
        pinky4,
    ...
    const findBone = (parent, name, index) => {
...
export function findSkinnedMeshes(model) {
    const meshes = [];
    model.traverse(obj => {
 ...
 * @returns Skeleton
 */
export function createSkeletonFromBone(bone) {
    const bones = [];
    bone.traverse(b => {
...
function findRootBone(bone) {
    let node = bone;
    while (node.parent && node.parent.isBone) {
        node = node.parent;
    }
...
export function getAPose(rightHand, _rightUpperArmPos) {
...
export default function avatarBoneMatching(asset) {
    ...
    hips.traverse(target => {
        ...
        const bone = mixamoVRMRigMap[boneName];
        if (bone) {
            bones[bone] = { node: target };
        }
    });
    ...
    const vrm = new VRM({
        humanoid,
        scene: scene,
        meta: { name: scene.children[0].name },
    });
...
export const enforceTPose = humanoid => {
