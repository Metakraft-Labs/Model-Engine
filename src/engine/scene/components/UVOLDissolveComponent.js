...
export const UVOLDissolveComponent = defineComponent({
    ...
    createDissolveMaterial(
        mesh,
        vertexHeaderTemplate = "#include <clipping_planes_pars_vertex>",
        vertexTemplate = "#include <fog_vertex>",
        fragmentHeaderTemplate = "#include <clipping_planes_pars_fragment>",
        fragmentTemplate = "#include <opaque_fragment>",
    ) {
        const material = mesh.material;
        const isShaderMaterial = "isShaderMaterial" in material;
        const texture = isShaderMaterial ? material.uniforms.map.value : material.map;
...
if (positionY < lowerOffset) {
  ...
  gl_FragColor.b = randomB;
     ...
     */
    updateDissolveEffect(entity, mesh, deltaTime) {
