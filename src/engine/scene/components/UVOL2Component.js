...
import { ECSState, ECSState as EngineState } from "../../../ecs/ECSState";
...
import { getState, NO_PROXY_STEALTH, none } from "../../../hyperflux";
import { isIPhone, isMobile } from "../../../spatial/common/functions/isMobile";
import {
    addObjectToGroup,
...
import { FORMAT_TO_EXTENSION } from "../constants/UVOLTypes";
...
export const calculatePriority = manifest => {
    ...
    for (let i = 0; i < textureTypes.length; i++) {
        ...
        if (videoTextures.length > 0 && !isMobile && !isMobileXRHeadset) {
        ...
        supportedTextures.sort((a, b) => {
            ...
            const aPixelPerSec =
                aData.frameRate *
                aData.settings.resolution.width *
                aData.settings.resolution.height;
            const bPixelPerSec =
                bData.frameRate *
                bData.settings.resolution.width *
                bData.settings.resolution.height;
    ...
    return [manifest, geometryTargets, textureTargets, useVideoTexture];
};

const getDefines = manifest => {
    const DEFINES = {
        baseColor: {
            USE_MAP: "",
            MAP_UV: "uv",
...
function sortAndMergeBufferMetadata(rangesState, gapTolerance) {
    ...
    for (let i = 1; i < ranges.length; i++) {
        ...
        const canMerge =
            ...
            (nextRange.start <= currentRange.end ||
                nextRange.start - currentRange.end <= gapTolerance);
        if (canMerge) {
            // Merge ranges
...
export const UVOL2Component = defineComponent({
    ...
    setStartAndPlaybackTime: (entity, newMediaStartTime, newPlaybackStartDate) => {
    ...
    canPlayThrough: (entity, start, end) => {
        ...
        if (!component.useVideoTexture) {
            for (const textureType of component.textureInfo.textureTypes)
                if (
                    !confirmBufferedRange(
                        component.textureInfo[textureType].buffered,
                        start,
                        end,
                        false,
                        MAX_TOLERABLE_GAP,
...
const loadDraco = url => {
    return new Promise((resolve, reject) => {
        ...
        getState(AssetLoaderState).gltfLoader.dracoLoader?.load(url, geometry => {
...
const loadTexture = (url, repeat, offset) => {
    return new Promise((resolve, reject) => {
        const startTime = performance.now();
...
const resolvePath = (path, manifestPath, format, target, index, textureType) => {
...
const createKey = (target, index, textureType) => {
    let key = target;
    if (textureType) {
        key += "_" + textureType + "_";
    }
...
function UVOL2Reactor() {
    ...
    const geometryBuffer = useMemo(() => new Map(), []);
    const textureBuffer = useMemo(() => new Map(), []);
    ...
    const material = useMemo(() => {
        ...
        let _material = new MeshBasicMaterial({ color: 0xffffff });
        if (manifest.type === 2) {
...
transformed.z += mix(keyframeA.z, keyframeB.z, mixRatio);
  ...
  vPosition = vec3( transformed );
    ...
    const defaultGeometry = useMemo(() => new SphereGeometry(3, 32, 32), []);
    ...
    useEffect(() => {
        ...
        const [sortedManifest, sortedGeometryTargets, sortedTextureTargets, useVideoTexture] =
            calculatePriority(component.data.get({ noproxy: true }));
        ...
        if (component.data.geometry.targets[sortedGeometryTargets[0]].totalSize) {
            const geometryBitrate =
                component.data.geometry.targets[sortedGeometryTargets[0]].totalSize.value /
                component.data.duration.value;
    ...
    useEffect(() => {
        if (!shadow) return;
        if (component.data.value.type === 2) {
            // TODO: Cast shadows properly with uniform solve
            shadow.cast.set(false);
            shadow.receive.set(false);
        } else {
    ...
    const fetchUniformSolveGeometry = (startTime, endTime, target) => {
        ...
        for (let i = startSegment; i <= endSegment; i++) {
            ...
            const metadata = { start, end, fetchTime: -1 };
            ...
            loadGLB(segmentURL)
                .then(({ mesh: segmentMesh, fetchTime }) => {
                    ...
                    const frameData = geometryBuffer.get(target);
                    const positionMorphAttributes = segmentMesh.geometry.morphAttributes.position;
                    const normalMorphAttributes = segmentMesh.geometry.morphAttributes.normal;
                    ...
                    if (
                        ...
                        mesh.geometry.attributes.position.array.length !==
                            segmentMesh.geometry.attributes.position.array.length
                    ) {
                    ...
                    if (!component.firstGeometryFrameLoaded.value) {
                        ...
                        repeat.copy(segmentMesh.material.map?.repeat ?? repeat);
                        offset.copy(segmentMesh.material.map?.offset ?? offset);
                ...
                .catch(() => {
                    const idx = component.geometryInfo.buffered
    ...
    const adjustGeometryTarget = () => {
        ...
        const totalFetchTime = component.geometryInfo.buffered.value.reduce(
            (acc, curr) => acc + curr.fetchTime,
            0,
        );
    ...
    const adjustTextureTarget = type => {
        ...
        const totalFetchTime = component.textureInfo[type].buffered.value.reduce(
            (acc, curr) => acc + curr.fetchTime,
            0,
        );
    ...
    const fetchGeometry = () => {
        ...
        const fetchEndTime = Math.min(
            fetchStartTime + maxBufferHealth,
            component.data.duration.value,
        );
        const target =
            component.geometryInfo.targets.value[component.geometryInfo.currentTarget.value];
        const targetData = component.data.value.geometry.targets[target];
    ...
    const fetchTextures = textureType => {
        const fetchStartTime = volumetric.currentTrackInfo.currentTime.value;
        const fetchEndTime = Math.min(
            fetchStartTime + maxBufferHealth,
            component.data.duration.value,
        ...
        for (let i = startFrame; i <= endFrame; i++) {
            ...
            if (!textureBuffer.has(textureType)) {
                textureBuffer.set(textureType, new Map());
            }
            const currentTextureBuffer = textureBuffer.get(textureType);
            ...
            if (confirmBufferedRange(bufferMetadata.value, start, end, true, MAX_TOLERABLE_GAP))
                continue;
    ...
    useEffect(() => {
        if (component.loadingEffectStarted.value && !component.loadingEffectEnded.value) {
            let headerTemplate = /\/\/\sHEADER_REPLACE_START([\s\S]*)\/\/\sHEADER_REPLACE_END/;
            let mainTemplate = /\/\/\sMAIN_REPLACE_START([\s\S]*)\/\/\sMAIN_REPLACE_END/;

            if (component.data.value.type !== 2 || 1 == 1) {
    ...
    const getFrame = (currentTime, frameRate, integer = true) => {
        const frame = currentTime * frameRate;
        return integer ? Math.round(frame) : frame;
    };

    const getAttribute = (name, currentTime) => {
        const currentGeometryTarget =
            component.geometryInfo.targets[component.geometryInfo.currentTarget.value].value;
        let index = getFrame(
            currentTime,
            component.data.value.geometry.targets[currentGeometryTarget].frameRate,
            false,
        );
        if (name === "keyframeA") {
    ...
    const setPositionAndNormal = (name, attr) => {
        setAttribute(name, attr.position);
        if (attr.normal) {
            setAttribute(name + "Normal", attr.normal);
        }
    };
    ...
    const setGeometry = (target, index) => {
        const frameData = geometryBuffer.get(target);
        const targetData = component.data.value.geometry.targets[target];
        ...
        if (!frameData || !frameData[index]) {
            ...
            if (targetData.format === "draco") {
                const geometry = frameData[index];
    ...
    const setMap = (textureType, texture) => {
        let oldTextureKey = "";
        ...
        if (mesh.material instanceof ShaderMaterial) {
            ...
            const material = mesh.materialBasicMaterial;
    ...
    const setTexture = (textureType, target, index, currentTime) => {
        const currentTextureBuffer = textureBuffer.get(textureType);
        if (!currentTextureBuffer) {
        ...
        if (!frameData || !frameData[index]) {
            ...
            for (let i = 0; i < targets.length; i++) {
                const _frameRate =
                    component.data.value.texture[textureType].targets[targets[i]].frameRate;
            ...
            const texture = frameData[index];
            setMap(textureType, texture);
        }
    };

    const updateUniformSolve = currentTime => {
        ...
        if (!keyframeA && !keyframeB) {
            ...
            const keyframeATime =
                keyframeAIndex / component.data.value.geometry.targets[keyframeATarget].frameRate;
            ...
            const keyframeBTime =
                keyframeBIndex / component.data.value.geometry.targets[keyframeBTarget].frameRate;
            ...
            if (frameData && frameData.length > 0) {
                for (const frameNo in frameData) {
                    ...
                    if (frameTime < currentTime - 0.5) {
                        const attribute = frameData[frameNo];
                        oldGeometry.setAttribute(
                            attribute.position.name + ".position",
                            attribute.position,
                        );
                        if (attribute.normal) {
                            oldGeometry.setAttribute(
                                attribute.normal.name + ".normal",
                                attribute.normal,
                            );
                        }
                        delete frameData[frameNo];
                        ...
                        relevantBufferIndex = component.geometryInfo.buffered.findIndex(tr => {
                            if (
                                tr.start.value <= frameTime &&
                                tr.end.value >= frameTime &&
                                tr.fetchTime.value > -1
                            ) {
                                return true;
                            }
                        });
                        if (relevantBufferIndex !== -1) {
                            component.geometryInfo.buffered[relevantBufferIndex].start.set(
    ...
    const updateNonUniformSolve = currentTime => {
        const geometryTarget =
            component.geometryInfo.targets[component.geometryInfo.currentTarget.value].value;
        const targetData = component.data.value.geometry.targets[geometryTarget];
        const geometryFrame = Math.round(
            currentTime * component.data.value.geometry.targets[geometryTarget].frameRate,
        );
        ...
        for (const target in component.data.value.geometry.targets) {
            ...
            if (frameData && frameData.length > 0) {
                for (const frameNo in frameData) {
                    ...
                    if (frameTime < currentTime - 0.5) {
                        if (targetData.format === "draco") {
                            const geometry = frameData[frameNo];
                            geometry.dispose();
                        } else if (targetData.format === "glb") {
                            const oldMesh = frameData[frameNo];
    ...
    const updateGeometry = currentTime => {
        if (component.data.value.type === 2) {
    ...
    const updateAllTextures = currentTime => {
        component.textureInfo.textureTypes.value.forEach(textureType => {
            updateTexture(textureType, currentTime);
        });
    };

    const updateTexture = (textureType, currentTime) => {
        const textureTarget =
            component.textureInfo[textureType].targets[
                component.textureInfo[textureType].currentTarget.value
            ].value;
        const textureFrame = Math.round(
            currentTime *
                component.data.value.texture[textureType].targets[textureTarget].frameRate,
        );
        ...
        for (const target in component.data.value.texture[textureType]?.targets) {
            ...
            const frameRate = component.data.value.texture[textureType]?.targets[target].frameRate;
            if (frameData && frameData.length > 0) {
                for (const frameNo in frameData) {
                    const frameTime = parseInt(frameNo) / frameRate;
                    if (frameTime < currentTime - 0.5) {
                        ...
                        relevantBufferIndex = component.textureInfo[textureType].buffered.findIndex(
                            tr => {
                                if (
                                    tr.start.value <= frameTime &&
                                    tr.end.value >= frameTime &&
                        ...
                        if (relevantBufferIndex !== -1) {
                            component.textureInfo[textureType].buffered[
                                relevantBufferIndex
                            ].start.set((parseInt(frameNo) + 1) / frameRate);
                        }
                    } else {
                        break;
                    }
    ...
    const update = () => {
        ...
        if (volumetric.autoPauseWhenBuffering.value) {
            ...
            if (
                (component.useVideoTexture.value || volumetric.hasAudio.value) &&
                media.readyState < 3
            ) {
                isWaitingNow = true;
            }
        ...
        startTransition(() => {
            ...
            if (_relevantBufferIndex !== -1) {
                _bufferedUntil = Math.min(
                    _bufferedUntil,
                    component.geometryInfo.buffered[_relevantBufferIndex].end.value,
                );
            }
            for (const textureType of component.textureInfo.textureTypes.value) {
                _relevantBufferIndex = component.textureInfo[textureType].buffered.findIndex(tr => {
                    if (tr.start.value <= _currentTime && tr.end.value >= _currentTime) {
                        return true;
                    }
                });
        ...
        if (
            volumetric.currentTrackInfo.currentTime.value > component.data.value.duration ||
            media.ended
        ) {
            if (
                component.data.deletePreviousBuffers.value === false &&
                volumetric.playMode.value === "loop"
            ) {
        ...
        if (!component.useVideoTexture.value) {
            ...
            if (!videoTexture.repeat.equals(repeat) || !videoTexture.offset.equals(offset)) {
                ...
                mesh.material.uniforms.mapTransform.value.copy(videoTexture.matrix);
            }
        }
