...
import { BoundingBoxComponent } from "../../../spatial/transform/components/BoundingBoxComponents";
...
export const AudioNodeGroups = new WeakMap();

export const createAudioNodeGroup = (el, source, mixbus) => {
    const audioContext = getState(AudioState).audioContext;
    const gain = audioContext.createGain();
    source.connect(gain);
    gain.connect(mixbus);
    const panner = audioContext.createPanner();
    const group = { source, gain, mixbus, panner };
...
export const MediaElementComponent = defineComponent({
    ...
    onSet: (entity, component, json) => {
        ...
        if (
            ...
            json.element !== component.element.get({ noproxy: true })
        )
            component.element.set(json.element);
    },
        ...
        startTransition(() => {
            if (typeof json.paths === "object") {
                ...
                const paths = json.paths.map(path =>
                    path.replace(".drcs", ".mp4").replace(".uvol", ".mp4"),
                );
            ...
            if (typeof json.resources === "object") {
                if (typeof json.resources[0] === "string") {
                    ...
                    component.resources.set(json.resources.map(resource => resource.path));
                }
            }

            if (typeof json.uiOffset === "object") {
                component.uiOffset.set(
                    new Vector3(json.uiOffset.x, json.uiOffset.y, json.uiOffset.z),
                );
            }
            ...
            if (
                ...
                if (typeof json.playMode === "number") {
                    switch (json.playMode) {
                        case 1:
                            component.playMode.set("single");
                            break;
                        case 2:
                            component.playMode.set("random");
                            break;
                        case 3:
                            component.playMode.set("loop");
                            break;
                        case 4:
                            component.playMode.set("singleloop");
                            break;
                    }
                } else {
...
export function MediaReactor() {
    ...
    useEffect(
        function updateTrackMetadata() {
            ...
            for (const [i, path] of paths.entries()) {
                if (path === "") continue;
                const assetClass = AssetLoader.getAssetClass(path).toLowerCase();
                const tempElement = document.createElement(assetClass);
    ...
    useEffect(
        function updateVolume() {
            ...
            const element = getOptionalComponent(entity, MediaElementComponent)?.element;
            if (!element) return;
...
const setUpMediaElement = (
    ...
    const assetClass = AssetLoader.getAssetClass(path).toLowerCase();
    setComponent(entity, MediaElementComponent, {
        element: document.createElement(assetClass),
    });
    const mediaElementState = getMutableComponent(entity, MediaElementComponent);
...
export const setupHLS = async (entity, url) => {
...
export function getNextTrack(currentTrack, trackCount, currentMode) {
    if (currentMode === "single" || trackCount === 0) return -1;
