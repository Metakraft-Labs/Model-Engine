export default class BufferData {
     ...
     */
    lowerBound(array, value, property = "startTime") {
     ...
     */
    upperBound(array, value, property = "endTime") {
        const length = array.length;
        let low = 0,
            high = length - 1,
            index = -1;
     ...
     *
     */
    updateEndTime(index, pending = false) {
        ...
        while (i < length && array[index].endTime >= array[i].startTime) {
            ...
            if (!pending) {
                this.bufferedRange[index].fetchTime =
                    this.bufferedRange[index].fetchTime + this.bufferedRange[i].fetchTime;
            }
    ...
    addRange(startTime, endTime, fetchTime, pending) {
        ...
        if (startTime > array[lb].endTime) {
            ...
            if (lb === length - 1) {
                ...
                if (!pending) {
                    array[length].fetchTime = fetchTime;
                }
            } else {
                array.splice(lb + 1, 0, { startTime, endTime });
                if (!pending) {
                    array[lb + 1].fetchTime = fetchTime;
                }
        ...
        if (!pending) {
            array[lb].fetchTime += fetchTime;
        }
        this.updateEndTime(lb);
    }

    removeBufferedRange(startTime, endTime) {
        this.removeRange(startTime, endTime, false);
    }

    removePendingRange(startTime, endTime) {
        this.removeRange(startTime, endTime, true);
    }

    removeRange(startTime, endTime, pending) {
        const array = pending ? this.pendingRange : this.bufferedRange;
        if (
            array.length === 0 ||
            startTime >= endTime ||
        ...
        if (lb < ub) {
            ...
            const ubFetchTime = pending ? 0 : array[ub].fetchTime;

            if (endTime < array[ub].endTime) {
                array[ub].startTime = endTime;
                if (!pending) {
                    array[ub].fetchTime =
                        (ubFetchTime * (array[ub].endTime - array[ub].startTime)) /
                        ubStoredDuration;
                }
            } else {
                array.splice(ub, 1);
            ...
            const lbFetchTime = pending ? 0 : array[lb].fetchTime;

            if (array[lb].startTime < startTime) {
                array[lb].endTime = startTime;
                if (!pending) {
                    array[lb].fetchTime =
                        (lbFetchTime * (array[lb].endTime - array[lb].startTime)) /
                        lbStoredDuration;
                }
            } else {
                array.splice(lb, 1);
            ...
            const fetchTime = pending ? 0 : array[index].fetchTime;
            const oldEndTime = array[index].endTime;

            if (endTime < oldEndTime) {
                array.splice(index + 1, 0, { startTime: endTime, endTime: oldEndTime });
                if (!pending) {
                    array[index + 1].fetchTime =
                        (fetchTime * (array[index + 1].endTime - array[index + 1].startTime)) /
                        storedDuration;
                }
            }
            ...
            if (array[index].startTime < startTime) {
                ...
                if (!pending) {
                    array[index].fetchTime =
                        (fetchTime * (array[index].endTime - array[index].startTime)) /
                        storedDuration;
                }
            } else {
    ...
    getInterSectionDurationInternal(array, startTime, endTime) {
        if (
            array.length === 0 ||
            startTime >= endTime ||
            endTime <= array[0].startTime ||
        ...
        for (let i = lb; i <= ub; i++) {
            duration +=
                Math.min(array[i].endTime, endTime) - Math.max(array[i].startTime, startTime);
        }

        return duration;
    }

    getIntersectionDuration(startTime, endTime) {
        const bufferedDuration = this.getInterSectionDurationInternal(
                this.bufferedRange,
                startTime,
                endTime,
            ),
            pendingDuration = this.getInterSectionDurationInternal(
                this.pendingRange,
                startTime,
                endTime,
            ),
    ...
    getNextMissing(currentTime) {
        ...
        while (true) {
            const maxOfBoth = Math.max(
                this.bufferedRange.length > 0
                    ? this.bufferedRange[this.bufferedRange.length - 1].endTime
                    : 0,
                this.pendingRange.length > 0
                    ? this.pendingRange[this.pendingRange.length - 1].endTime
                    : 0,
            );
            ...
            const pendingLb = this.lowerBound(
                this.pendingRange,
                this.bufferedRange[bufferedLb].endTime,
                "startTime",
            );
            if (pendingLb === -1) {
                return this.bufferedRange[bufferedLb].endTime;
