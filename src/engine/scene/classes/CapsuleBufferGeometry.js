...
export class CapsuleGeometry extends BufferGeometry {
    ...
    constructor(
        ...
        this.indices = new BufferAttribute(
            new (this.indexCount > 65535 ? Uint32Array : Uint16Array)(this.indexCount),
            1,
        );
    ...
    calculateVertexCount() {
        const count =
            ...
            (this.heightSegments + 1 + this.capsBottomSegments + this.capsTopSegments);
        return count;
    }
    ...
    calculateIndexCount() {
        const count =
            this.radialSegments *
            (this.heightSegments + this.capsBottomSegments + this.capsTopSegments) *
            2 *
            3;
        return count;
    }
    ...
    generateTorso() {
        ...
        for (y = 1; y <= this.heightSegments; y++) {
            ...
            const radius =
                sinAlpha *
                ((y * (this.radiusBottom - this.radiusTop)) / this.heightSegments + this.radiusTop);
            ...
            for (x = 0; x <= this.radialSegments; x++) {
                ...
                vertex.y =
                    this._halfHeight +
                    cosAlpha * this.radiusTop -
                    (y * cone_height) / this.heightSegments;
                vertex.z = radius * cosTheta;
                this.vertices.setXYZ(this._index, vertex.x, vertex.y, vertex.z);
        ...
        for (y = 1; y <= this.capsBottomSegments; y++) {
            ...
            const a =
                Math.PI / 2 - this.alpha - (Math.PI - this.alpha) * (y / this.capsBottomSegments);
        ...
        for (x = 0; x < this.radialSegments; x++) {
            for (
                y = 0;
                y < this.capsTopSegments + this.heightSegments + this.capsBottomSegments;
                y++
            ) {
