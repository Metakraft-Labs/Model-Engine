...
import LogarithmicDepthBufferMaterialChunk from "../../../spatial/renderer/constants/LogarithmicDepthBufferMaterialChunk";
...
class TrailRenderer extends Mesh {
    ...
    static createMaterial(vertexShader, fragmentShader, customUniforms = {}) {
        customUniforms.trailLength = { type: "f", value };
        customUniforms.verticesPerNode = { type: "f", value };
        customUniforms.minID = { type: "f", value };
        customUniforms.maxID = { type: "f", value };
        customUniforms.dragTexture = { type: "f", value };
        customUniforms.maxTrailLength = { type: "f", value };
        customUniforms.textureTileFactor = { type: "v2", value };
    ...
    //   customUniforms.texture = { type: 't', value }
    ...
    updateNodePositionsFromOrientationTangent(nodeIndex, nodeCenter, orientationTangent) {
        ...
        for (let i = 0; i < this.localHeadGeometry.length; i++) {
            ...
            positions.setXYZ(
                positionIndex,
                transformedHeadVertex.x,
                transformedHeadVertex.y,
                transformedHeadVertex.z,
            );
        }
    ...
    connectNodes(srcNodeIndex, destNodeIndex) {
        ...
        for (let i = 0; i < this.localHeadGeometry.length - 1; i++) {
            ...
            const faceIndex =
                (srcNodeIndex * this.FacesPerNode + i * FacesPerQuad) * IndicesPerFace;
