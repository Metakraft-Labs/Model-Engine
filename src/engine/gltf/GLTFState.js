...
export const GLTFSourceState = defineState({
     ...
     * @returns
     */
    load: (source, uuid = MathUtils.generateUUID(), parentEntity = UndefinedEntity) => {
...
export const GLTFSnapshotState = defineState({
    ...
    receptors: {
        onSnapshot: GLTFSnapshotAction.createSnapshot.receive(action => {
            ...
            // whatever snapshots have been undone
        ...
        onRedo: GLTFSnapshotAction.redo.receive(action => {
            ...
            state.index.set(
                Math.min(state.index.value + action.count, state.snapshots.value.length - 1),
            );
        }),
            ...
            for (let i = 0; i < snapshot.nodes.length; i++) {
                ...
                if (node.children && node.children.length) {
                    for (const child of node.children) {
                        ...
                        if (childUUID === uuid) {
                            return node.extensions?.[UUIDComponent.jsonID];
                        }
                    }
                }
            }
    ...
    cloneCurrentSnapshot: source => {
        const state = getState(GLTFSnapshotState)[source];
        return JSON.parse(JSON.stringify({ source, data: state.snapshots[state.index] }));
    },

    injectSnapshot: (srcNodeUUID, srcSnapshotID, dstNodeUUID, dstSnapshotID) => {
        ...
        for (const child of childEntities) {
            ...
            const childNode = snapshot.data.nodes?.find(
                node =>
                    node.extensions?.[UUIDComponent.jsonID] === getComponent(child, UUIDComponent),
            );
        ...
        parentSnapshot.data.scenes[0].nodes = parentSnapshot.data.scenes[0].nodes.filter(
        ...
        for (const node of newNodes) {
            ...
            for (const child of node.children) {
                ...
                const childIndex = newNodes.findIndex(
        ...
        const childIndices = roots.map(root =>
            parentSnapshot.data.nodes.findIndex(node => node === root),
        );
        const parentNode = parentSnapshot.data.nodes?.find(
            node => node.extensions?.[UUIDComponent.jsonID] === dstNode,
        );
        ...
        dispatchAction(
            GLTFSnapshotAction.createSnapshot({ source: dstSnapshotID, data: parentSnapshot.data }),
        );
        dispatchAction(GLTFSnapshotAction.unload({ source: srcSnapshotID }));
    },
});

export const EditorTopic = "editor";

const ChildGLTFReactor = props => {
...
export const DocumentReactor = props => {
    const nodeState = useHookstate(getMutableState(GLTFNodeState)[props.documentID]);
    if (!nodeState.value) return null;
    return (
        <>
            {Object.entries(nodeState.get(NO_PROXY)).map(
                ([uuid, { nodeIndex, childIndex, parentUUID }]) => (
                    <ParentNodeReactor
                        key={uuid}
...
const NodeReactor = props => {
    ...
    const entity = useHookstate(() => {
        const uuid = node.extensions.value?.[UUIDComponent.jsonID];
...
// const MeshReactor = (props: { nodeIndex; documentID; entity }) => {
...
// const PrimitiveReactor = (props: { primitiveIndex; nodeIndex; documentID; entity }) => {
//   const documentState = useHookstate(getMutableState(GLTFDocumentState)[props.documentID])
//   const nodes = documentState.nodes as State<GLTF.INode[]>
//   const node = nodes[props.nodeIndex]!
