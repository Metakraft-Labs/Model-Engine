...
} from "../../../engine/scene/components/ObjectGridSnapComponent";
...
import { AvatarRigComponent } from "../../../engine/avatar/components/AvatarAnimationComponent";
...
function bboxDistance(bbox1, bbox2, matrixWorld1, matrixWorld2) {
...
function findClosestAxis(axis, candidates) {
...
function alignToClosestAxis(matrix1, matrix2) {
    ...
    const rightAxes = dstAxes.filter(
        axis =>
            Math.abs(axis.dot(dstForward)) < Number.EPSILON &&
            Math.abs(axis.dot(dstUp)) < Number.EPSILON,
    );
...
function boundedTranslation(bbox1, bbox2, matrixWorld1, matrixWorld2) {
...
function calculateTranslation(bbox1, bbox2) {
    ...
    for (const axis of ["x", "y", "z"]) {
        if (bbox1.max[axis] < bbox2.min[axis]) {
            ...
            const box1Pts = [
                bbox1.min[axis],
                (bbox1.min[axis] + bbox1.max[axis]) / 2,
                bbox1.max[axis],
            ];
            const box2Pts = [
                bbox2.min[axis],
                (bbox2.min[axis] + bbox2.max[axis]) / 2,
                bbox2.max[axis],
            ];
...
function setHelperLayer(entity, layer) {
...
function setHelperColor(entity, color) {
    const helper = getOptionalMutableComponent(entity, BoundingBoxHelperComponent);
    if (helper) {
...
function resetHelperTransform(entity) {
...
export const ObjectGridSnapState = defineState({
    ...
    apply: () => {
        ...
        for (let i = 0; i < selectedEntities.length; i++) {
            ...
            const helperEntity = getOptionalComponent(
                selectedEntity,
                BoundingBoxHelperComponent,
            )?.entity;
            if (!helperEntity) {
                console.warn(
                    `ObjectGridSnapSystem:apply No Helper entity found for selected entity: ${selectedEntity}`,
                );
                continue;
            }
...
export const ObjectGridSnapSystem = defineSystem({
    ...
    execute: () => {
        ...
        for (let i = 0; i < selectedEntities.length; i++) {
            ...
            const selectedMatrixWorld = getComponent(
                selectedEntity,
                TransformComponent,
            ).matrixWorld;
            ...
            for (const candidateEntity of nonSelectedEntities) {
                ...
                const candidateMatrixWorld = getComponent(
                    candidateEntity,
                    TransformComponent,
                ).matrixWorld;
            ...
            for (const candidateEntity of closestEntities) {
                ...
                const rotationMatrix = alignToClosestAxis(
                    selectedMatrixClone,
                    candidateMatrixWorld,
                );
