...
export default function PaginatedList({ list, element, options }) {
    ...
    return (
        <>
                ...
                {list?.length > 0 && (
                    <div className="grid grid-cols-7 gap-4">
                        ...
                        {[-2, -1, 0, 1, 2].map(idx => {
                            ...
                            if (
                                !(
                                    currentPage.value + idx < 0 ||
                                    currentPage.value + idx >= list.length / countPerPage
                                )
                            )
                                return (
                        ...
                        <div className="col-span-1 flex justify-center">
                            <Button
                                ...
                                onClick={() =>
                                    currentPage.set(
                                        Math.min(
                                            Math.floor((list.length - 1) / countPerPage),
                                            Math.max(0, currentPage.value + 1),
                                        ),
                                    )
                                }
            ...
            {(pageView.value[0] === pageView.value[1] ? list : list.slice(...pageView.value)).map(
                (val, index) => {
