...
import { isHandleConnected, useChangeNode } from "../../../visualScript/VisualScriptUIModule";
import { useModifyNodeSocket } from "../../../visualScript/hooks/useModifyNodeSocket";
...
export const Node = ({ id, data, spec, selected, specGenerator }) => {
    ...
    if (canAddInputs) {
        handleAddNodeSocket = useModifyNodeSocket(
            id,
            "inputs",
            "increase",
            canAddInputs.defaultValue ?? 1,
        );
    ...
    const collapsedHandlePositions = [];
    ...
    return (
        ...
        >
            ...
            {!collapsed && (
                <div
                    className={twMerge("flex-col", "border-grey-800 flex gap-4 p-1.5", borderColor)}
                >
                    {pairs.map(([input, output], ix) => (
                        <div
                            key={ix}
                            className="node-container-row flex flex-row justify-between gap-0.5 p-0"
                        >
                            {input && (
                                <InputSocket
                                    {...input}
