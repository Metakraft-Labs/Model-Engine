...
const PickerItem = ({ label, node, onPickNode, position, instance, color }) => {
    ...
    useEffect(() => {
        if (!isExpanded && !childNodes) {
            const children = createPickerNodes(node, onPickNode, position, instance);
    ...
    const handleNodeClick = () => {
        if (isLeafNode) {
            const paneBounds = document
                .getElementById(VisualScriptPanelTab.id)
    ...
    const finalColor = colors[color][0].slice(2);

    return (
        <>
            <div
                onClick={handleNodeClick}
                className="flex w-full items-center justify-between py-1 pl-2"
            >
...
const NodePickerNode = ({ nodes, onPickNode, position, instance }) => {
    ...
    return (
        <div className="flex h-48 flex-col overflow-x-hidden overflow-y-scroll">
            {initialTreeNodes}
        </div>
    );
};
...
export const NodePicker = ({ position, onPickNode, onClose, filters, specJSON }) => {
    ...
    position.x =
        position.x - width > 0
            ? position.x + width > paneBounds.width
                ? (position.x -= width)
                : position.x
            : position.x;
    position.y =
