...
import { mergeBufferGeometries } from "../../../spatial/common/classes/BufferGeometryUtils";
...
async function meshStandardMaterialComparator(group, a, b) {
    ...
    if (
        ...
        return Promise.all([
            meshBasicMaterialComparator(group, a, b),
...
export default class MeshCombinationGroup {
    ...
    static async combineMeshes(rootObject) {
        ...
        for (const material of materials) {
            matPromises.push(
                dedupeTexture(imageHashes, textureCache, material.map).then(
                ...
                dedupeTexture(imageHashes, textureCache, material.aoMap).then(
                    texture => (material.aoMap = texture),
                ),
                dedupeTexture(imageHashes, textureCache, material.normalMap).then(
                    texture => (material.normalMap = texture),
                ),
                dedupeTexture(imageHashes, textureCache, material.emissiveMap).then(
                    texture => (material.emissiveMap = texture),
                ),
                dedupeTexture(imageHashes, textureCache, material.lightMap).then(
                    texture => (material.lightMap = texture),
                ),
            );
        }
    ...
    async _tryAdd(object) {
        ...
        if (
            !(
                compareMaterial &&
                (await compareMaterial(this, this.initialObject.material, object.material))
            )
        ) {
            return false;
        }
    ...
    _combine() {
        ...
        for (const mesh of this.meshes) {
            ...
            const hasNegativeScale =
                matrixWorld.elements[0] * matrixWorld.elements[5] * matrixWorld.elements[10] < 0;
