...
import CubemapCapturer from "../../../engine/scene/classes/CubemapCapturer";
import {
...
const getScenePositionForBake = entity => {
    if (entity) {
        const transformComponent = getComponent(entity, TransformComponent);
        return transformComponent.position;
    }
    let entityToBakeFrom;
...
export const uploadBPCEMBakeToServer = async entity => {
 ...
 * @returns
 */
export const bakeEnvmapTexture = async position => {
    const renderer = getComponent(Engine.instance.viewerEntity, RendererComponent).renderer;
 ...
 * @returns
 */
export const uploadCubemapBakeToServer = async (name, data) => {
    ...
    const currentSceneDirectory = getState(EditorState)
        ...
        .slice(0, -1)
        .join("/");
    const urlList = await uploadProjectFiles(
        projectName,
