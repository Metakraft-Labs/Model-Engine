...
import { getState, NO_PROXY } from "../../../hyperflux";
import { TransformComponent } from "../../../spatial";
import { CameraComponent } from "../../../spatial/camera/components/CameraComponent";
import { Axis, Q_IDENTITY, Vector3_Zero } from "../../../spatial/common/constants/MathConstants";
...
const _positionMultiStart = {};
const _quaternionStart = new Quaternion();
const _quaternionMultiStart = {};
const _scaleStart = new Vector3();
const _scaleMultiStart = [];
...
export function gizmoUpdate(gizmoEntity) {
    ...
    setVisibleComponent(
        gizmo.gizmo[TransformMode.translate],
        gizmoControl.mode === TransformMode.translate,
    );
    setVisibleComponent(
        gizmo.gizmo[TransformMode.rotate],
        gizmoControl.mode === TransformMode.rotate,
    );
    setVisibleComponent(
        gizmo.gizmo[TransformMode.scale],
        gizmoControl.mode === TransformMode.scale,
    );

    setVisibleComponent(
        gizmo.helper[TransformMode.translate],
        gizmoControl.mode === TransformMode.translate,
    );
    setVisibleComponent(
        gizmo.helper[TransformMode.rotate],
        gizmoControl.mode === TransformMode.rotate,
    );
    setVisibleComponent(
        gizmo.helper[TransformMode.scale],
        gizmoControl.mode === TransformMode.scale,
    );

    setVisibleComponent(
        gizmo.picker[TransformMode.translate],
        gizmoControl.mode === TransformMode.translate,
    );
    setVisibleComponent(
        gizmo.picker[TransformMode.rotate],
        gizmoControl.mode === TransformMode.rotate,
    );
    setVisibleComponent(
        gizmo.picker[TransformMode.scale],
        gizmoControl.mode === TransformMode.scale,
    );
    ...
    handles = handles.concat(
        getComponent(gizmo.picker[gizmoControl.mode], GroupComponent)[0].children,
    );
    handles = handles.concat(
        getComponent(gizmo.gizmo[gizmoControl.mode], GroupComponent)[0].children,
    );
    ...
    for (const handle of handles) {
        ...
        // Hide disabled axes
        handle.visible =
            handle.visible && (handle.name.indexOf(TransformAxis.X) === -1 || gizmoControl.showX);
        handle.visible =
            handle.visible && (handle.name.indexOf(TransformAxis.Y) === -1 || gizmoControl.showY);
        handle.visible =
            handle.visible && (handle.name.indexOf(TransformAxis.Z) === -1 || gizmoControl.showZ);
        handle.visible =
            handle.visible &&
            (handle.name.indexOf(TransformAxis.E) === -1 ||
                (gizmoControl.showX && gizmoControl.showY && gizmoControl.showZ));
...
export function planeUpdate(gizmoEntity) {
    ...
    _v1.copy(Axis[TransformAxis.X]).applyQuaternion(
        space === TransformSpace.local ? gizmoControl.worldQuaternion : Q_IDENTITY,
    );
    _v2.copy(Axis[TransformAxis.Y]).applyQuaternion(
        space === TransformSpace.local ? gizmoControl.worldQuaternion : Q_IDENTITY,
    );
    _v3.copy(Axis[TransformAxis.Z]).applyQuaternion(
        space === TransformSpace.local ? gizmoControl.worldQuaternion : Q_IDENTITY,
    );
...
export function controlUpdate(gizmoEntity) {
    ...
    if (
        ...
        gizmoControl.pivotEntity.value == undefined
    )
        return; // need pivot Entity if more than one entity is controlled
    const targetEntity =
        gizmoControl.controlledEntities.value.length > 1
    ...
    if (camera.isOrthographicCamera) {
        ...
        gizmoControl.eye.set(
            camera.position.clone().sub(gizmoControl.worldPosition.value).normalize(),
        );
    }
}
...
function pointerHover(gizmoEntity) {
    ...
    const gizmoVisual = getComponent(
        gizmoControlComponent.visualEntity.value,
        TransformGizmoVisualComponent,
    );
    const picker = getComponent(
        gizmoVisual.picker[gizmoControlComponent.mode.value],
        GroupComponent,
    )[0];
    const targetEntity =
        ...
        if (planeIntersect) {
            ...
            if (
                ...
                for (const cEntity of gizmoControlComponent.controlledEntities.value) {
                    ...
                    currenttransform.matrix.decompose(
                        _cMultiStart,
                        _cQuaternionStart,
                        _cScaleStart,
                    );
...
function applyTranslate(
    ...
    _offset
        .applyQuaternion(
            space === TransformSpace.local && axis !== TransformAxis.XYZ
                ? _quaternionStart
    ...
    newPosition.copy(
        _offset.add(pivotControlledEntity ? _positionMultiStart[entity] : _positionStart),
    );
...
function applyScale(entity, pointStart, pointEnd, axis, scaleSnap, pivotControlledEntity = false) {
    ...
    newScale
        .copy(pivotControlledEntity ? _scaleMultiStart[entity] : _scaleStart)
        .multiply(_tempVector2);
...
function applyRotation(entity, gizmoControlComponent, axis, space) {
    ...
    const ROTATION_SPEED =
        20 /
        gizmoControlComponent.worldPosition.value.distanceTo(
    ...
    if (axis === TransformAxis.XYZE) {
        gizmoControlComponent.rotationAxis.set(
            _offset.cross(gizmoControlComponent.eye.value).normalize(),
        );
        gizmoControlComponent.rotationAngle.set(
            _offset.dot(
                _tempVector
                    .copy(gizmoControlComponent.rotationAxis.value)
                    .cross(gizmoControlComponent.eye.value),
            ) * ROTATION_SPEED,
        );
        ...
        if (_tempVector.length() === 0) {
            ...
            gizmoControlComponent.rotationAngle.set(
                _offset.dot(_tempVector.normalize()) * ROTATION_SPEED,
            );
        }
    }
...
function applyPivotRotation(entity, pivotToOriginMatrix, originToPivotMatrix, rotationMatrix) {
    _tempMatrix.compose(
        _positionMultiStart[entity],
        _quaternionMultiStart[entity],
        _scaleMultiStart[entity],
    );
    _tempMatrix
...
function pointerMove(gizmoEntity) {
    ...
    if (mode === TransformMode.scale) {
        ...
        axis === TransformAxis.E ||
        axis === TransformAxis.XYZE ||
    ...
    gizmoControlComponent.pointEnd.set(
        planeIntersect.point.sub(gizmoControlComponent.worldPositionStart.value),
    );
    ...
    if (mode === TransformMode.translate) {
        ...
        if (
            ...
            for (const cEntity of gizmoControlComponent.controlledEntities.value) {
                ...
                const newDistance = _positionMultiStart[cEntity]
                    .clone()
                    .sub(_positionStart.clone())
                    .multiply(_tempVector2);
        ...
        EditorControlFunctions.rotateObject(
            [entity],
            [_tempEuler.setFromQuaternion(newRotation, "XYZ")],
        );
        if (
            gizmoControlComponent.controlledEntities.value.length > 1 &&
            gizmoControlComponent.pivotEntity.value !== UndefinedEntity
            ...
            const rotationMatrix = _tempMatrix
                ...
                .makeRotationAxis(
                    space === TransformSpace.local
                        ? Axis[axis]
                              .clone()
                              .applyQuaternion(gizmoControlComponent.worldQuaternion.value)
                        : gizmoControlComponent.rotationAxis.value,
                    gizmoControlComponent.rotationAngle.value,
                );
            for (const cEntity of gizmoControlComponent.controlledEntities.value) {
...
export function onGizmoCommit(gizmoEntity) {
    ...
    if (gizmoControlComponent.dragging && gizmoControlComponent.axis !== null) {
        ...
        if (!getState(ObjectGridSnapState).enabled) {
            EditorControlFunctions.commitTransformSave(
                gizmoControlComponent.controlledEntities.get(NO_PROXY),
            );
        } else {
...
export function onPointerLost(gizmoEntity) {
