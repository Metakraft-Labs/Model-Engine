...
import { UVOL1Component } from "../../../../engine/scene/components/UVOL1Component";
import { UVOL2Component } from "../../../../engine/scene/components/UVOL2Component";
import { getState } from "../../../../hyperflux/StateFunctions";
 ...
 * @constructor
 */
export const VolumetricNodeEditor = props => {
    ...
    const [textureTargets, setTextureTargets] = React.useState({});
    useEffect(() => {
        if (!uvol2) {
            return;
        }
        const textureTypes = uvol2.textureInfo.textureTypes.value;
        const _textureTargets = {};
        textureTypes.forEach(textureType => {
            _textureTargets[textureType] = [];
    ...
    return (
        ...
        >
            <InputGroup
                name="useLoadingEffect"
                label={t("editor:properties.volumetric.lbl-useLoadingEffect")}
            >
                <BooleanInput
            ...
            {(hasComponent(props.entity, UVOL2Component) ||
                hasComponent(props.entity, UVOL1Component)) && (
            ...
            <InputGroup name="Play Mode" label={t("editor:properties.media.playmode")}>
                ...
                />
                {volumetricComponent.paths &&
                    volumetricComponent.paths.length > 0 &&
                    volumetricComponent.paths[0] && (
                        <Button
            ...
            {hasComponent(props.entity, UVOL2Component) && (
                <>
                    ...
                    {Object.keys(textureTargets).map((textureType, index) => {
                        return (
                            <InputGroup
                                name={`${textureType} target`}
                                label={`${textureType} target`}
                                key={index}
                            >
                                <SelectInput
                                    key={props.entity}
...
function VolumetricCurrentTimeScrubber(props) {
    const { t } = useTranslation();
    const volumetricComponent = useComponent(props.entity, VolumetricComponent);
        ...
        <InputGroup name="CurrentTime" label={t("editor:properties.media.lbl-currentTime")}>
            <Scrubber
                ...
                onScrubEnd={value => {
                    ...
                    const uvol2Component = getOptionalMutableComponent(
                        props.entity,
                        UVOL2Component,
                    );
                    if (
