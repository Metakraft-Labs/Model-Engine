...
import capitalizeFirstLetter from "../../../../common/src/utils/capitalizeFirstLetter";
import ColorInput from "../../../../primitives/tailwind/Color";
import { generateDefaults } from "../../../../spatial/renderer/materials/constants/DefaultArgs";
...
14: "entity"*/
    return (
        <>
            {Object.entries(_defaults).map(([k, parms]) => {
                ...
                return (
                    ...
                    >
                        {(() => {
                            if (!(k in values)) {
                                console.warn(
                                    `Key "${k}" not found in ParameterInput values object`,
                                );
                                return null;
                            }
                            ...
                            switch (parms.type) {
                                case "boolean":
                                    return (
                                        <BooleanInput value={values[k]} onChange={setArgsProp(k)} />
                                    );
                                case "entity":
                                case "integer":
                                case "float":
                                    return (
                                        <NumericInput value={values[k]} onChange={setArgsProp(k)} />
                                    );
                                case "string":
                                    return (
                                        <TextField value={values[k]} onChange={setArgsProp(k)} />
                                    );
                                case "color":
                                    return (
                                        <ColorInput value={values[k]} onChange={setArgsProp(k)} />
                                    );
                                case "texture":
                                    return (
                                        <TexturePreviewInput
