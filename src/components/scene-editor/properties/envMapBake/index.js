...
export const EnvMapBakeNodeEditor = props => {
    ...
    const renderEnvMapBakeProperties = () => {
        const renderedProperty = DefaultEnvMapBakeSettings.map((element, id) => {
            if (
                element.label == "Realtime Settings" &&
                bakeComponent.bakeType.value == EnvMapBakeTypes.Realtime
            ) {
    ...
    return (
        ...
        >
            <Button
                className="my-1 ml-auto px-10"
                onClick={() => uploadBPCEMBakeToServer(props.entity)}
            >
            ...
            <InputGroup name="Bake Resolution" label="Bake Resolution">
                <SelectInput
                    options={bakeResolutionTypes.map(resolution => ({
