...
const variants = {
    outlined: " ",
    underlined:
        "bg-transparent border-0 border-b rounded-none placeholder:text-neutral-200 placeholder:text-[17px]",
};
...
const Input = forwardRef(
    (
        ...
        const containerClass =
            variant === "outlined"
                ? "bg-theme-surface-main relative h-full w-full"
        ...
        return (
            <div className={twcontainerClassName}>
                ...
                {description && (
                    <p className="self-stretch text-xs text-theme-secondary">{description}</p>
                )}
                {error && (
