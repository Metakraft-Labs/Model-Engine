...
const variants = {
    ...
    outline:
        "border border-solid border-theme-primary bg-theme-surface-main dark:bg-theme-highlight text-theme-primary",
    danger: "bg-red-500",
    success: "bg-teal-700",
    ...
    (
        ...
        return (
            <button ref={ref} role="button" disabled={disabled} className={twClassName} {...props}>
                {StartIcon && (
                    <span className={twMerge("mx-1", iconContainerClassName)}>{StartIcon}</span>
                )}
                {children && (
                    <span
                        className={twMerge(
                            "mx-1",
                            fullWidth ? "mx-1 w-full" : "",
                            textContainerClassName,
                        )}
                    >
                        {children}
                ...
                {EndIcon && (
                    <span className={twMerge("mx-1", iconContainerClassName)}>{EndIcon}</span>
                )}
            </button>
        );
    },
