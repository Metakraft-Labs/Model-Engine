...
function adjustOffset(rect, offsetX, offsetY, thresholdX = 10, thresholdY = 15) {
    let x = offsetX,
        y = offsetY;
    if (rect.top < 0) {
...
const ShowContextMenu = ({ anchorEvent, children, ...props }) => {
    ...
    useEffect(() => {
        ...
        offset.set(
            adjustOffset(ref.current.getBoundingClientRect(), offset.x.value, offset.y.value),
        );
    }, []);
        ...
        >
            <div
                ...
                style={{
                    all: "unset",
                    position: "fixed",
                    left: offset.x.value + "px",
                    top: offset.y.value + "px",
                }}
            >
                {children}
            </div>
