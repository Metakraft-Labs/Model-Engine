...
const FeathersChildReactor = (props = { serviceName }) => {
    const fetch = () => {
...
export const useGet = (serviceName, params = {}) => {
...
export function useMutation(serviceName, forceRefetch = true) {
    ...
    return useMemo(
        () => ({
            ...
            patch,
            remove,
            datatate: state.data.value,
            statustate: state.status.value,
            errortate: state.value.error,
        }),
        [create, update, patch, remove, state],
    );
}

function useMethod(_state, action, serviceName) {
    return useCallback(
        _args => {
            return "";
            // const service = API.instance.service(serviceName);
